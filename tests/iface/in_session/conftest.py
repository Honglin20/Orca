...
**sidechain 守护清理**（SPEC-B v4 B2）：同样 detach 起 ``sidechain_daemon``，本 fixture
**扩展**清理至 ``sidechain_daemon`` 进程（同 cwd 比对 pattern，与 chart 一致）。

本 autouse fixture 在每个测试后**仅清理** cwd 落在该测试 ``tmp_path** 下的守护进程（/proc
遍历，读 ``/proc/<pid>/cmdline`` 过滤 ``chart_daemon`` / ``sidechain_daemon`` + ``/proc/<pid>/cwd``
readlink 比对）。按 cwd 而非命令行参数匹配：``--tape`` 是相对路径（daemon 继承测试 cwd），
cmdline 不含 tmp_path；但 cwd symlink 解析后即测试 tmp_path，唯一标识本测试的守护。不影响
并发其它测试（pytest-xdist 各 worker 独立 tmp_path）。
"""
...
# 守护模块名 → 清理（chart + sidechain；新加守护仅扩此 tuple）。
_DAEMON_MODULES = (
    "orca.iface.in_session.chart_daemon",
    "orca.iface.in_session.sidechain_daemon",
)


@pytest.fixture(autouse=True)
def _cleanup_daemons_after_test(tmp_path: Path):
    """每个测试后：kill cwd 落在本测试 tmp_path 下的 chart/sidechain 守护进程。
    ...
    for entry in proc_dir.iterdir():
        ...
        try:
            ...
            continue
        # 任意守护模块命中即清。
        if not any(mod in cmdline for mod in _DAEMON_MODULES):
            continue
        try:
