...
覆盖意图（非仅行为）：
  - start_run（非 resume）→ chart socket 文件存在（2026-07-08：socket 走 ``<tmp>/orca-<hash>.sock``，
    不再在 runs 目录）
...
def _runs_dir(tmp_path: Path) -> Path:
    """runs_dir（tape/jsonl 落盘处）。

    2026-07-08 短路径化后，chart socket 不再放 runs 目录（走 ``chart_sock_path`` 的
    ``<tmp>/orca-<hash>.sock``），故 runs_dir 长度不再受 SOCK_PATH_MAX 约束——用 pytest
    tmp_path 即可。
    """
