...
> Orca NAS 三件套的完整版。在 `nas-hp-search` 的超参轴（宽度、深度、核）之上增加 **block 类型轴**，使搜索空间覆盖组件级选择（如 MBConv / TransformerEncoder），并以 LLM 评估节点在 Pareto 前沿上做最终综合选择。7 节点，最重但最完整。
...
`nas-agent-pipeline` 读取用户模型，经重量级优化器（展平、应用 `optimize_rules`、架构分类）生成包含 block 组件维度的完整弹性超网，训练后在「超参 × block 类型」的扩展搜索空间上运行 NSGA-II，最后由 LLM 评估节点综合精度、延迟与结构合理性选出最优架构，并生成再训练/微调脚本。

### 1.2 架构与流程

该 workflow 为 7 节点线性链。与轻量版的差异在于：节点 1 采用重量级优化器、节点 2/7 引入可视化、节点 6 由 LLM 驱动评估。

```
 model_optimizer            生成完整 Elastic 超网（重量级）
 (pytorch-model-optimizer)  展平模型 → <base>_flat.py；应用 optimize_rules；
                            分类 NAS 架构；生成含 block 组件维度的超网；
                            经子 agent 迭代修复
        ↓
 viz_describe               推 C1 基线表 + C2 超网/搜索空间表
 (nas-viz)                  （push_describe.py）
        ↓
 ...
 (nas-search-pipeline)
        ↓
 train_runner               真实执行训练 + 搜索
 (nas-train-runner)
        ↓
 evaluator                  【LLM 评估，完整版独有】读 AGENTS.md + Pareto 前沿
 (内联 prompt)              → 选最优架构 → 生成 retrain.sh/finetune.sh
                            → final_report.md + selection_summary.json
        ↓
 viz_finalize               推 C5 最终 Pareto 图 + C6 漏斗图
 (nas-viz)                  （push_pareto_final.py + push_funnel.py）
```
...
| 参数 | 类型 | 默认 | 说明 |
|---|---|---|---|
| `model_path` | string | — | 模型入口 |
| `project_root` | string | — | 项目根目录 |
| `output_dir` | path | 推断 | 留空→`llm_artifacts/<model_name>/` |

**输出**：`final_report.md`（最优架构报告，含 block 组成/参数量/延迟/精度对比）+ `selection_summary.json` + `retrain.sh`/`finetune.sh` + 可视化（C1/C2/C5/C6）。workflow 产出为 `evaluator` 节点的 `result`。
...
不同 block 类型的架构在参数量、延迟、精度上的差异非线性且难以由单一标量指标刻画。NSGA-II 产出的 Pareto 前沿仍可能含多个在纯指标上接近、但结构可解释性迥异的候选。本 workflow 在节点 6 引入 LLM 评估作为**模型中介的选择步骤**：它消费搜索产出的 Pareto 前沿与 `selection_summary.json`，综合纯指标无法捕捉的结构属性（如可解释性、与已知优良模式的吻合度）做最终决策。需强调该步骤不参与搜索算法本身，仅作用于搜索结果的后处理选择。
...
节点 6 为内联 prompt 驱动的 LLM 节点，其契约约束为：必须读取搜索产出的 `AGENTS.md` 与 Pareto 前沿、必须运行 `nas-select-architecture` 并持久化 `selection_summary.json`、必须生成 `retrain.sh`/`finetune.sh` 与 `final_report.md`。LLM 在纯指标（参数量、延迟、精度）之上，引入对 block 组成与结构合理性的综合判断，输出最优架构及其选择依据。
...
典型产出为：超网训练 → 搜索 `(width, depth, block_type)` 组合 → LLM 评估 → 最优架构。`final_report.md` 含参数量/延迟/精度/block 组成对比，并附 LLM 的选择依据。
...
block 类型轴的引入使搜索空间显著扩张，对超网训练时长与 NSGA-II 评估预算要求更高；block 类型间的权重共享亦较同质 block 更难均衡。LLM 评估节点的选择具不确定性，需以 `selection_summary.json` 与可复现的 Pareto 前沿作为可审计依据。当预设的 block 候选集仍无法触及目标时，应转用 `agent-struct-exploration`，由 LLM 在开放结构空间上提议任意改写。
...
节点 6 的内联 prompt 约束：必读 `AGENTS.md` 与 Pareto 前沿、必运行 `nas-select-architecture`、必持久化 `selection_summary.json`、必生成 `retrain.sh`/`finetune.sh`/`final_report.md`。LLM 输出的选择须可由 `selection_summary.json` 中记录的 Pareto 前沿与高权衡选择结果审计。
