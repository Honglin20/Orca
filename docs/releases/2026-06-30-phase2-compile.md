...
> phase 5 单轨化迁移后：③⑤（after 校验）随 `Node.after` 删除而废除，新增 ⑩⑪⑬，
> 现为 9 项（①②④⑥⑦⑧⑨⑩⑪⑬）。下列为 phase 2 当时交付的 8 项记录。

① name 非空+全局唯一 ② entry 存在 ③ after 引用有效 ④ routes[].to 有效（node 或 `$end`）
⑤ after 静态边无环（原 Kahn 算法；routes 条件边回指=合法循环，不校验）
...
- **鲁棒性**：边界全显式处理——空 workflow / route 自环 / foreach 空 source / route 空目标 / `workflow.input['x']` 订阅写法 —— 全部 fail loud 成 `ConfigurationError`，无裸异常（review 验证）。（phase 5 后 after 边界用例随字段删除而移除，替换为 parallel 组边界用例。）
