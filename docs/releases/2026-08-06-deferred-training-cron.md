...
3. **Step 3 status 推导**（fail loud 关键）：
   ...
   （`pid_alive AND .cron_registered.flag` 双条件）< `failed`。**双条件防 M1**：
...
- **TerminateNode.status 仅 `success` / `failed`**（schema `Literal["success", "failed"]`，无
  `pending`）。按任务 brief 选 `status: success` + reason 文案明示 deferred 语义，避免被误判
  workflow 失败。**未**新增 `deferred` 枚举（schema 改动超出本 task 范围，留作未来 suggestion
  给用户定夺）。
...
- Step 3 status 推导 5 场景 smoke test 全过：scenario A（stale eta + dead pid + no flag → failed，
  **M1 regression 通过**）/ B（pid alive + flag → detached）/ C（pid alive 无 flag → failed，
...
一轮 code-review（agent `a6a671debe2e3f84f`）发现 3 must-fix + 8 should-fix + 4 optional。

- **must-fix（3 全修）**：
  - M1：`detached_signal = pid_alive OR eta_marker` 掩盖 failed → 改 2a 清 deferred markers +
    Step 3 用 `pid_alive AND cron_registered.flag` 双条件。
  - M2：Step 2e `python3 -c '...f\"...\"...'` 是 Python 语法错（bash 单引号下 `\"` 字面到 python
    无效）→ 改 `python3 - <<'PY'` heredoc + 文件读 eta。
  - M3：Step 0a（多名字扫描）vs Step 3（search_config 单路径）ckpt 解析漂移 → 0a 写
    `.ns_run_train_ckpt_resolved.txt` marker，Step 3 优先读此 marker。
- **should-fix（8 全修）**：S1 cron PATH 注入、S2 `at` 限制文档化、S3/S5 Step 2c/2a 加 `set -e`、
  S4 cron_registered.flag 原子耦合、S6 resume-pending 显式 LOG_PATH、S7 `2>/dev/null 2>/dev/null`
  typo、S8 T_MIN 提取后显式校验。
- **optional（O1 采纳 / O2-O4 跳过）**：O1 改用 `{{ inputs | tojson }}` 替代硬编码 5 字段（DRY）；
  O2（Step 3 ckpt torch.load 校验）跳过——0a reuse 已校验 + 下游 ns_run_search 缺 ckpt fail loud
  双兜底；O3（per_epoch=60s）是设计选择，本 note 标注；O4（yaml description 缩短）跳过。
