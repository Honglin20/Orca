...
## [2026-09-08] perf(web): 列表性能止血 + 治本 P1~P4（scope=all 分页游标 / 目录指纹粗判 / 详情 tail 窗口 / 启动预热）（commit `10e4c28`）
...
## [2026-08-17] docs(research): 剪枝算法对比 + NAS×量化×剪枝联合压缩调研报告（commit `cabf2d1`）

deep-research 深度调研（106 子代理 / 24 一手来源 / 25 论断 3 票对抗验证）产出 [docs/research/2026-08-17-pruning-and-nas-quant-joint-compression.md](../research/2026-08-17-pruning-and-nas-quant-joint-compression.md)：剪枝四设计轴分类学 + 九大准则族对比 + 训练策略（one-shot/iterative、global/local、KD>finetune 恢复排序）+ 工具生态 + LLM 时代（Minitron 配方/剪枝 vs 从零训预算口径）+ NAS×剪枝×量化三范式（顺序/联合/弹性）+ 稀疏×量化交互（渐进强度假说、2:4+INT8 顺序与坑）+ 面向 Orca 现有 11 个压缩 workflow 的三阶段落地建议（prune-channel-sweep 升级路线 / compress-pipeline 顺序编排 / puzzle MIP 吸纳剪枝率与位宽决策变量）。校验 agent 复核 6 处标注/转写错误已全修。

## [2026-08-13] refactor(puzzle): 终端 reporter 化——删 7 个 terminate 节点，in-session 全兼容（commit `12164eb`）

in-session 引擎只支持 `kind: agent`（`step.py _check_agent_node`），路由到 terminate 抛 `unsupported_node_kind` 崩 run——puzzle 对齐 nas-supernet-v2/v3 的 reporter 模式：删全部 7 个 terminate 节点（16 → 9 节点），失败分支统一收敛到 `pz_report`（唯一终端 reporter），`pz_report` 改读磁盘 first-match 判终态（新 `emit_report.py` + `check_report.sh`；run.sh 成功路径才跑 gate，失败路径跳过；agent.md 零跨节点 output 引用；final_status.json 失败路径补写 U6 契约），outputs 全读 pz_report.output（失败路径 StrictUndefined 安全）。6 个上游 agent.md + pz_expand 遗留 + workflow-checklist 文案同步。验收：tars validate 全 13 workflow 0/0；emit_report 11 终态 synthetic 全对；test_puzzle_measure_baseline 31 + 相关 puzzle 82 + compile 215 + terminate 引擎 14 全过。详见 [release note](../releases/2026-08-13-puzzle-terminal-reporter.md)。
...
## [2026-08-13] feat(puzzle): pz_materialize 节点 —— optimized_flat 自包含最优架构交付件（commit `02db9dd`）
...
## [2026-08-13] refactor(nas-supernet-v3): 删 AGENTS.md——信息无损融进对应节点（commit `947551e`）
...
## [2026-08-13] feat(nas-supernet-v3): target E2E 全链跑通 + 通用规则提取（commit `04b9e95`）
...
## [2026-08-12] feat(nas-supernet-v3): retrain 拆分——ns3_retrain_script（纯生成）+ ns3_retrain（执行）（commit `bd05ce9`）
...
## [2026-08-11] feat(in-session): resume 任意 failed run + 可配 recoverable 升格上限（默认 20）

两件事：① `_RECOVERABLE_ESCALATE_AT=3` 硬编码 → `Workflow.recoverable_max_attempts`（默认 20，`ge=1`，workflow 级可配），同节点连续 recoverable 失败（output_schema_mismatch/agent_blocked）撞上限才升格；常量及全引用（code+docstring）清零。② **`orca next --run-id X` 在 failed 态自动 resume 失败节点**（= `state.current_node`）：emit `workflow_resumed`+`node_started`、reducer 翻 failed→running、重建 marker、喂回失败历史 + 计数清零、续跑到 completed；run 重新活跃 + web 可见如续跑。SPEC-REVIEW 12 issue 闭环；E2E 第一轮发现 `workflow_resumed` 未传播到下游终态判定（chart/sidechain daemon 秒退、web meta.status stale failed、stop 误短路）→ SPEC §2.4 补强 + 第二轮修 4 模块（`chart_daemon._watch_terminal` 跨 poll `terminated` 标志、`run_manager._scan_terminal_type/_probe_head_and_terminal/_scan_meta_overview`、`_tape_probe.scan_terminal`、cli gate cancelled/completed 诚实 reply）。TEST-AGENT 两轮真机 E2E 全 PASS（AC1/3/4/5/6/7/8/9/10/11）零产品 bug。`fdb4977`。详见 [release note](docs/releases/2026-08-11-resume-failed-runs-and-configurable-escalation.md) + [SPEC](docs/specs/2026-08-11-resume-failed-and-configurable-escalation.md)。

## [2026-08-11] fix(nas-supernet): v2 审计 S4b 三项 SDD——expand 归因 marker / resume attempt N / retrain ckpt 核验（v1+v2+v3）

v2 审计 6 项中的 3 项 SDD 项（另 3 项 ⑧⑦⑥ 属 S4a `d768879`）。**③** expand unsupported 归因 marker 化（仅 v2+v3，v1 无 report 节点是 dead code；Step 0 rm 协议防跨 run/attempt 残留 + Step 1.5 unsupported 分支 `printf 'true'` best-effort 不阻塞 emit；report 双信号 marker OR summary 子串兜底，绕开 LLM 自由文本脆弱子串匹配，不违零跨节点 output 铁律）。**④** Step R 两分支重建 attempt N（RESUME_SEARCH=latest-mtime 号 / RESUME_HEAL=max+1，禁一刀切 max+1 误杀在跑搜索；v1+v2+v3 同改 + setsid 评注订正）。**⑤** retrain ckpt 相对路径核验 → verify-and-close（三版本 `status.sh` case-block 已绝对化 + `emit_result.py` `os.path.join(ad,...)`，零旁路写相对，零代码改动）。spec-reviewer conditional-pass 3 Blocker（Q1/Q2/Q5-Q6）+ hardening 全闭环。36 新测试（AC-③1/②2/③3 + 跨分支优先级 + AC-④1 a/b/c + 空 .search_pid + AC-④2）+ 81 chart 回归 + 58 struct 回归过；code-reviewer 闭环（0 Blocker / 2 建议 + 1 trivial 全补）。`9f8c301`。详见 [release note](docs/releases/2026-08-11-v2-audit-attribution-resume-ckpt.md)。
