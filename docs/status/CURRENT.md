...
- [ ] **target / mnist_trf E2E 重跑——用户 2026-08-12 指示跳过**：环境 deepseek 余额 0 + 无 ANTHROPIC key，in-session LLM 后端不可用。U2a spike 已用 Claude 代跑证 LLM flatten 可行（strict-load 零 missing，0 fix-loop）；U2b recall 14 用例 env-gated skip。代码层 U0-U5 + code-review 闭环全完成（tars validate 0/0，57 passed）。**target 真验 + deepseek 复核留作余额恢复后补**。
