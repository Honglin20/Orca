...
### 11.8 Skip to Agent signature 裁定（P4，2026-07-02）—— 与 SPEC §9 task1 末段的偏离

**偏离/裁定**：SPEC §9 task1 末段写「Decide the clean signature: e.g.
`request_interrupt(ireq, answer=("skip", guidance), skip_target: str | None = None)` or carry
the target inside the InterruptRequest/answer. Pick cleanly (Rule 7 if ambiguous) — prefer
extending the existing `answer` tuple or adding an explicit optional param; document.」

**裁定（Rule 7）**：采用「explicit optional param」——`request_interrupt(ireq, answer, skip_target=None)`，
`skip_target` 作为独立 keyword 参数，**不**折进 `answer` tuple（即 `answer` 仍是 `(action, guidance)` 二元组）。

**理由**：
1. **语义正交**：`answer=(action, guidance)` 是「用户的中断意图 + 纠偏话」（continue/skip/abort 三分支
   共享的 cohesive 语义）；`skip_target` 是 **SKIP 专属**的派生信号（仅 action=="skip" 时有意义）。
   把 SKIP 专属字段塞进三分支共享的 tuple 会让 continue/abort 调用方困惑（要传什么？None？）。
2. **向后兼容**：`answer` 形状不变（wave-1 全部调用方 / 测试无需改）；`skip_target=None` 默认值
   让旧调用方零改动。若折进 tuple（变 3-tuple），所有 `answer=(action, guidance)` 调用点都要改。
3. **多壳路径退化清晰**：web/mcp 路径（`answer=None`）的 `skip_target` 自然 None（多壳 resolve
   不带此参），退化行为在签名层面可见。

`_handle_interrupt` 返回值相应从 `str`（action）改为 `tuple[str, str | None]`（action, skip_target），
让 `_drive_loop` 能拿到目标。SPEC §9 task1 的「`answer` tuple or explicit param」二选一以本裁定为准。

**UX pattern 裁定（同节）**：采用 pattern A（InterruptModal → app 推 NodeSelectModal），**非**
pattern B（InterruptModal 内嵌选 node 视图）。理由：保持 InterruptModal 单一职责（continue/skip/abort
