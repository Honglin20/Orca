...
> **v5 相对 v4 变化**：**删 `describe` 命令**（用户决策：冗余）→ `orca list` 直接返 `{name, description, inputs_schema}`（一个命令给 skill 选 wf + 抽 inputs 的全部信息）。命令数回到 **7**。**〔2026-07-17 演进〕**：inputs_schema 移出 `orca list`（选 wf 阶段噪音，单 wf 可达 20+ 字段），改由 `orca <wf>` 不带 `--inputs` 按需带出（抽 inputs 用）。仍无 describe 命令——schema 挂启动命令而非新命令。其余同 v4（A2 拆 setup vs gate / A1 MCP migration / B1 禁用 transform / B5 删 orca.ts / step 3-5 拆分）。**A5 修正：nudge 不再 defer merge，改 step 2b 做（idle/Stop hook 提醒，精确判定卡住，不推进）**。
...
in-session = 路径 B（主 session 驱动），跨 CC/opencode/NGA/CAC 统一一套 orca 接口；前端只装 skill（删 command）；setup 全删；命令行后端全归 tars（命令名变量化）；旧接口清零。**唯一真相源 + 单一接口 + 架构整洁**。
...
**B 固有代价 + 缓解**：押 LLM（deepseek/CC 已验证够；弱模型用 tars）；静默卡住靠 `status` 显示 `last_next_at`+elapsed（兜底可见）+ **nudge hook（step 2b 做，§4.4）**：idle/Stop 时有活跃 run → 提醒主 session 调 next（精确判定，不推进）。
...
| 命令 | 作用 | 谁调 |
|---|---|---|
| `orca list` | 列 wf，**只返 `{workflows:[{name, description}]}`**（选 wf 用，据 description 匹配；inputs_schema 不在此，由 `orca <wf>` 按需给，**不读 YAML**） | skill/LLM |
| `orca <wf> [--inputs '{...}']` | 不带 `--inputs` → 返 `{name, description, inputs_schema}`（抽 inputs，不启动）；带 → 启动（bootstrap → entry prompt + 驱动协议） | 主 session |
...
- **无 `describe` 命令** —— 选 wf 经 `orca list`（只 name+description）；inputs_schema 经 `orca <wf>`（不带 `--inputs`）按需带出。各命令各司其职，无需单独 describe。

### 2.2 保留字黑名单（MS1）

wf name 禁取 `list/next/status/stop/open/doctor` + tars 命令名（`run/serve/ps/...`）→ compile fail loud。

### 2.3 返回契约

| 命令 | 返回 |
|---|---|
| `orca <wf>` | 不带 `--inputs` → `{name, description, inputs_schema:[{name,type,description}]}`（不启动）；带 `--inputs` → `{run_id, prompt, prompt_file, done:false}` |
...
| `orca list` | `{workflows:[{name, description}]}`（**无 has_setup**，B3；inputs_schema 移至 `orca <wf>`） |
| `orca stop` | `{run_id, ok, done}` |

### 2.4 可见性守门

`orca --help`/错误/skill 禁出现 tars 命令名；测试断言 `orca --help` 含 7 命令、不含 tars。

---

## 3. tars（命令行 / 后端，命令名变量化）

### 3.1 命令族

`tars run/serve/open/ps/install/validate/mcp/executor/list/logs/wait/resume`（后端 + 运维 + 工具；实施时核实实际数对齐标题）。`install` 单 binary 归 tars；`list` 与 `orca list` 共享 catalog 单一实现。

### 3.2 命令名变量化

env `ORCA_BACKEND_CMD`（默认 `tars`），安装/打包按变量生成命令脚本，改 env 重装即换名。
...
删 `templates/opencode/command/orca/*.md`（4 文件）；新增入口 skill（**TARS 品牌**：用户面 = TARS，底层用 orca CLI 引擎；一套 SKILL.md），skill 内联注入主 session。

### 4.2 skill 流程（不读 YAML，三步）

SKILL.md 教主 session：
1. `orca list` → 拿 wf 列表（**只 name + description**）→ LLM 据 description 选 wf。
2. `orca <wf>`（不带 `--inputs`）→ 拿 inputs_schema → 据此从用户意图抽 inputs。
3. `orca <wf> --inputs '{...}'` → 读驱动协议 → 派 Task 子代理 → 自调 `orca next --run-id --output` 循环到 done。

**skill 绝不读 YAML**（选 wf 经 `orca list`，inputs_schema 经 `orca <wf>`，各命令各司其职）。
...
`tars install --target cc|opencode|cac|nga` 装同一份随包 skill 到对应目录；**step 6 用户澄清**（2026-07-15）：CAC ≡ Claude Code（`.claude`→`.cac`，其余同）、NGA ≡ opencode（`.opencode`→`.nga`，其余同）。install 阶段两家族**全套统一装**（不只 skill）：cc 家族（cc/cac）= skill + nudge Stop-hook（`hooks/orca-nudge.sh` + `settings.json` 声明）；opencode 家族（opencode/nga）= skill + plugin `orca.ts`（idle nudge 载体）+ `opencode.json` 声明（路径指各自 root——dotdir 或 opencode user scope 的 config 根）。
...
2b. 入口切 skill（下一个实施）：
   (1) 建入口 skill（TARS 品牌，一套 SKILL.md，三步指导）+ teams install --target cc/opencode/cac/nga 四平台落点代码
   (2) orca list 返 inputs_schema（name+type+description）+ 删 has_setup（B3）【无 describe 命令】**〔2026-07-17 演进：inputs_schema 移出 list 改由 orca <wf> 带出，见 §2.1/§4.2/§11〕**
...
3a. inputs 代填 skill 完善（〔2026-07-17 演进：inputs_schema 改由 `orca <wf>` 不带 --inputs 带出；skill 三步 = list 选 wf → `orca <wf>` 看 schema → `orca <wf> --inputs` 启动，见 §4.2〕）
...
5. `orca list` 只返 `{name, description}`（选 wf 用）；inputs_schema 由 `orca <wf>`（不带 `--inputs`）按需带出（抽 inputs 用）。各命令各司其职，无 describe。
6. setup 全删（§6.1，execute-phase gate 校验保留 A2）+ MCP migration。
7. tars 命令名变量化（env ORCA_BACKEND_CMD）。
...
- `orca --help` 含 7 命令、不含 tars 命令名、**不含 describe**。
- `orca list` 返 `{workflows:[{name, description}]}`（无 inputs_schema / has_setup；inputs_schema 移至 `orca <wf>` 不带 --inputs）。
...
- `tars install --target cac` 装上 **cc 家族全套**（CAC≡cc）：skill + nudge Stop-hook（`.cac/hooks/orca-nudge.sh` + `.cac/settings.json` 声明）。
- `tars install --target nga` 装上 **opencode 家族全套**（NGA≡opencode）：skill + plugin `orca.ts`（idle nudge）+ `opencode.json` 声明（路径指 `.nga`）。
