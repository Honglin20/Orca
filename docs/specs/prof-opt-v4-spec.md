...
│   │   ├── assert_shadow.py           # 不变
│   │   ├── PROFILER_CONTRACT.md       # 头段入口措辞随 D-V4-20 更新（外部脚本直连机制退役 → profiling 经 mfu-analyzer + mfu_adapter）；四件套字段 schema 不变
│   │   ├── placeholder_profiler.py    # 保留（真 profiler 未到位的默认；v2 搁置物）
│   │   ├── mfu_benchmark.py           # 用户的真评测脚本载体（D-V4-20：文件名锁定，内容随用户提供替换；mfu 模式下经 mfu-analyzer 子代理调用部署件）
│   │   ├── analyze.py                 # 不变
│   │   ├── predict_delta.py           # 不变
...
│   │   ├── mfu_adapter.py             # 【新增·D-V4-20】mfu 模式确定性转换：读 profile_dir 内 mfu_benchmark.py 原始产物（schedule_result.json 的并行 cycles = canonical makespan + taskgraph CSV / subgraph json）→ PROFILER_CONTRACT 四件套；缺文件/缺字段/makespan 不一致 fail loud；不跑评测（评测归 mfu-analyzer）
...
└── subagents/prof-opt/
    ...
    └── mfu-analyzer.md                # 【新增·D-V4-20】mfu 模式 profiling 执行层（用户 MFU Bottleneck Analyzer 适配 Orca 协议：跑部署件 mfu_benchmark.py + 三阶段解析 + 瓶颈报告落盘；H1-H6 硬规则；原始产物只读）
tests/
...
- **inputs**（D-V4-20 后 = **14 个**）：v4 原有 12 个中 **`profile_script_path` 退役删除**；`probe_epochs`（变体短训深度 k）与 `full_train_epoch_cap`（基线与 winner 共用上限，生效值落 full_train_budget）description 更新保留；**新增三个 [advanced]**：`npu_chip`（string default ""——空 = placeholder 本地估算模式[默认]；"6613"/"1951" = mfu 真评测模式，非空且非枚举值启动即 fail loud）、`npu_precision`（string default "INT8"，枚举 INT8/INT16/AMP，仅 mfu 模式消费）、`npu_core_num`（integer default 1，枚举 1/2/4，仅 mfu 模式消费）。
- **nodes**（8 个）：
...
**7 个 agent.md**（po_gate 是 script 节点）全部含：frontmatter / Resource Anchors / Path Iron Rules / Subagent Call Protocol（仅声明实际 dispatch：flatten→memory-verifier；contract→paradigm-verifier；baseline→business-logic-analyst + mfu-analyzer[仅 npu_chip 非空]；propose→bottleneck-analyst+structure-proposer+variant-implementer + mfu-analyzer[仅 npu_chip 非空]；probe/full_train/report 不派）/ Lazy Loading / Workflow 步骤 / Validation / Output。节点本地 `scripts/`（check_business_logic.sh / run_latency_recheck.sh / check_prerequisites.sh）经 `$ORCA_AGENT_RESOURCES` 直跑。prompt 洁净 = 强制验收（§6）。

## 4. 各节点专属契约（草稿锚 + 实现增量）

| 节点 | 草稿锚 | 实现增量 |
|---|---|---|
| po_flatten | v3.5 §3.1 | reuse 语义澄清（复测轮）：跨 run 复用前提 = **零晋升史**——promoted 后 shadow 已前进而 BASELINE.lock 仍锚原始基线，锁「可读但不匹配」多为该设计行为而非错误（失败文案指向 fresh_start=true 重建）；锁不可读/损坏 = 真错误（失败文案两态可区分）。**D-V4-20**：reuse_check.sh 第 4 参从 profile_script_path 存在性校验改为 `npu_chip` 枚举校验（空 或 6613/1951，非法值 exit 3） |
...
| bottleneck-analyst | base/profile/ 四件套 + 全部原始产物 + bottleneck_report.json（+ mfu_bottleneck_report.md 在场时增读） | `base/bottleneck_analysis.json`（零重复机械字段；保序子集映射） | `check_bottleneck.py`；失败重派 1 次仍败 → error |
| mfu-analyzer | onnx 路径 + profile_dir + report_path + chip/precision/core_num（调用方提供） | 原始产物落 profile_dir（只读不动）+ `mfu_bottleneck_report.md`（首行哨兵；评测失败按 H5 仍出报告）| 节点侧机械校验：关键原始产物在场（schedule_result.json 等）+ 报告存在 + 哨兵；缺失重派 1 次 → error |
...
- **7 个 agent.md + 7 个 subagent md + 4 个 references**（structural-levers/probe_protocol/full_train_protocol/report_format）逐一单独分 agent 审查（受众翻转通读）。**D-V4-20 增量改动文件（mfu-analyzer.md 新建 + po_baseline/po_propose agent.md + yaml + reuse_check.sh）须复审后更新对应 verify/cleanliness/ 记录。**
...
- 残留 grep（范围 = `workflows/agents/ + workflows/subagents/prof-opt/ + workflows/prof-opt.yaml`，命中 = 0）：v3.5 词表（`mnist_kd`、`playground`、`prof_opt_demo`、ns3/psu/kd-nas/nas-supernet 词边界、`prof-opt-design-draft|prof-opt-v2-design-draft|prof-opt-v4-design-draft`、`docs/specs`、`D:\?Projects`、`/mnt/d`、`spec-review`、`SPEC-R1`）+ 增补退役物（`run_verify`、`baseline_proxy_acc`、`baseline_ref`、`profile_script_path`、`perturb_ckpt`、`playbook`、`ref-input`、`auto-trained`）。注：`mfu_adapter.py` 自 D-V4-20 起是正式交付物，**移出**退役词表。

## 7. E2E 验收（claude 后端 + tars skill，两项目；驱动形态继承 v3.5 SPEC §7）

> **执行归属（2026-08-26 用户拍板）**：本地（Orca 仓库侧）E2E 仅覆盖 **placeholder 模式**（`npu_chip` 空）；**mfu 模式 E2E 由用户在带 NPU 访问的真机自跑**（远程提交依赖真实 mfu 服务）。下方验收线对两模式通用；mfu 模式在真机执行时按同断言集核对（①中 rendered 比对不含 profiling 参数；mfu 侧新增断言 = mfu_bottleneck_report.md 在场含哨兵 + 四件套 makespan == schedule_result.json 并行 cycles）。

- **inputs 钉**：`full_train_epoch_cap=2, probe_epochs=1`（k<full 使停止判据非退化；秒级训练下 kill 可能落在自然结束之后——严格 kill 语义归单测，E2E 侧以 stop_status 终态计数披露行使性）、`max_rounds=2`、`latency_reduction_min` 显式传 0.3~0.5（收紧 round-1 即 full-train 的空间）。**loop 为观察项非硬断言**（placeholder 下 round-1 达标或 exhausted 时结构性不可达——`gate_decide.py` loop 需 not exhausted 且未达标；硬断言 = gate exit 0 + decision ∈ 4 合法值 + 路由 == decision 的机械映射）。placeholder 场景阈值调整落点 = `run_latency_recheck.sh` 调用行实参（**非 input**）。
