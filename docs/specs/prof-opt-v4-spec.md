...
| 节点 | 草稿锚 | 实现增量 |
|---|---|---|
| po_flatten | v3.5 §3.1 | reuse 语义澄清（复测轮）：跨 run 复用前提 = **零晋升史**——promoted 后 shadow 已前进而 BASELINE.lock 仍锚原始基线，锁「可读但不匹配」多为该设计行为而非错误（失败文案指向 fresh_start=true 重建）；锁不可读/损坏 = 真错误（失败文案两态可区分） |
