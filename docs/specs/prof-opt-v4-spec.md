...
│   ├── po_flatten/       { agent.md, scripts/{reuse_check.sh, check_flatten.sh, extract_user_pkg.sh} }   # 不变
...
│   ├── po_propose/       { agent.md, scripts/{run_latency_recheck.sh}, references/{structural-levers.md} }   # 【重构】；run_latency_recheck = 原 po_verify/scripts/run_verify.sh 迁移改名（331 行确定性逻辑，阈值实参 100/1/0.5 调用行显式；skip 键 = verdict.json 存在性——打回修复后复测前必须删该 vid verdict.json）
