...
1. **入口/prompt 用 AgentHarness 的极简哲学**
   - `entry` 显式声明唯一入口；控制流统一由 `routes` 单指针表达（phase 5 单轨化）
...
3. **节点类型只保留必要的 4 种 kind**：`agent` / `script` / `set` / `foreach`；
   静态并行靠顶层 `parallel` 组（diamond 分叉+合并）

### 融合对照表

> phase 5 单轨化迁移后：废除 `after` 双轨，统一为 `routes` 单指针 + `parallel` 组。
> 下表「Orca 决策」反映迁移后的最终态。

| 特性 | AgentHarness | Conductor | Orca 决策 | 理由 |
|---|---|---|---|---|
| 入口声明 | `after=[]` 约定 | `entry:` 显式 | **`entry:` 显式** | 显式优于隐式 |
| 串行控制流 | `after` 静态依赖 | `depends_on` | **`routes` 单指针**（first-match-wins）| 一种机制统一线性/分支/回环/终止，消除 join 歧义 |
| 条件路由 | `on_pass`/`on_fail` 二元 | `routes:` + Jinja2 | **`routes:` + Jinja2** | 多路 + 表达式 |
| 静态并行 | `after` 多源汇聚（隐式）| `parallel:` 块 | **`parallel:` 顶层独立列表**（显式）| DAG 分叉+合并需显式声明，消除「多前置跑几次」歧义 |
...
class Route(BaseModel):
    ...
    to: str                             # 目标 node 名 / parallel 组名 / "$end"
```
...
- 所有 node 的 `name` + parallel 组名共享命名空间，全局唯一
- `entry` 必须是某个 node 的 name（不能是 parallel 组名）
- `routes[].to` 引用的必须是已定义的 node 名 / parallel 组名 / `"$end"`
- entry 沿 routes 前向边（parallel 组展开为 branches）必须可达终态；
  routes 回指是合法循环（单指针模型不做静态无环校验，死锁改运行时检测）
...
### 6.2 静态并行（parallel 组：diamond 分叉+合并）

phase 5 单轨化：DAG 分叉+合并不能用 routes 表达（routes 每步只去一个 target），
必须用顶层 `parallel` 组。entry=researcher_a，a 完成后 routes 到 parallel 组
`researchers_merge`，组 branches 并行执行（已执行的 branch 幂等跳过），组完成后
按组 routes 推进到 synthesizer。
...
nodes:
  ...
  - name: researcher_b                 # 由 parallel 组驱动执行（不再是隐式入口）
...
|---|---|---|---|
| Node 模型 | 一个 AgentDef 塞所有字段 | Agent 类 + after | 基类 + kind 特化（discriminated union）；phase 5 去 after，routes 单轨 |
| 路由 | routes + 双引擎 | on_pass/on_fail | routes + 单一 Jinja2（first-match-wins）|
| 并行 | parallel: 块 | after 汇聚 | parallel 顶层独立列表（diamond 分叉+合并）|
