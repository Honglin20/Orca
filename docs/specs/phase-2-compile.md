...
### 8 项校验规则（每项 fail loud，错误信息精确）

#### ① name 全局唯一 + 非空（phase 1 遗留）
- error: `name` 为空字符串（phase 1 把 name 设为可选）
- error: 两个 node 同名
- 消息示例：`node 名重复：'optimizer' 出现 2 次`

#### ② entry 存在（phase 1 遗留）
- error: `wf.entry` 不是任何 node 的 name
- 消息：`entry 'optimizer' 不存在于 nodes 中`

#### ③ after 引用有效（phase 1 遗留）
- error: `after` 列表里的某个 name 不存在
- 消息：`node 'trainer' 的 after 引用了不存在的 node 'foo'`

#### ④ routes[].to 引用有效（phase 1 遗留）
- error: `routes[].to` 不是真实 node 也不是 `"$end"`
- 消息：`node 'reviewer' 的 route 引用了不存在的目标 'xxx'`

#### ⑤ after 静态边无环（DAG）
- **只校验 after 静态边**（routes 是条件边，回指是合法循环，不校验环）
- 用 Kahn 拓扑排序检测
- error: 检测到环
- 消息：`检测到 after 静态依赖环：optimizer → trainer → optimizer`

#### ⑥ 从 entry 至少有一条路能到 $end（可达性）
- 从 entry 出发，沿 after + routes 边走，必须能到达某个 `routes[].to == "$end"` 的节点
- error: 死胡同（走到某节点无路可走，且该节点没有 routes 到 $end）
- warning: 某个 node 从 entry 不可达（孤立节点，可能用户忘了接线）
...
| 环检测 | inode 级 sub-workflow 环检测 | after 静态边 Kahn 拓扑 | routes 回指不校验（合法循环）|
...
### 6.3 校验验收（8 项，每项有对应的错误用例测试）
- [ ] ① name 重复/空 → ConfigurationError 含 "重复" 或 "空"
- [ ] ② entry 不存在 → ConfigurationError
- [ ] ③ after 引用不存在 → ConfigurationError
- [ ] ④ routes.to 不存在 → ConfigurationError
- [ ] ⑤ after 环 → ConfigurationError 含环路径
- [ ] ⑥ entry 到不了 $end → ConfigurationError
...
3. **Kahn 拓扑**：环检测用标准 Kahn 算法，对 after 静态边构图
4. **可达性**：DFS/BFS 从 entry 出发，沿 after + routes 边，看能否到 `$end`
...
|---|---|---|
| 顶层 node name 非空唯一 | ① | phase 2 实现 |
| entry 存在 | ② | phase 2 实现 |
| after/路由引用有效 | ③④ | phase 2 实现 |
| after 静态边无环 | ⑤ | phase 2 实现 |
...
2. entry 存在
3. after 引用有效
4. routes[].to 引用有效（node 或 $end）
5. after 静态边无环（Kahn 拓扑）
6. entry 可达 $end
7. Jinja2 浅校验（node 名存在；用 jinja2.meta.find_undeclared_variables）
8. foreach.source 的 node 存在（浅校验）
...
8. after 静态边无环，但 routes 回指是合法循环（不校验环）

## 产出要求
1. 上述 3 个 compile 文件
2. `tests/compile/test_parser.py` —— 解析 + prompt 加载 + 3 examples 通过
3. `tests/compile/test_validator.py` —— 8 项校验各正/反例 + errors 聚合 + warnings
