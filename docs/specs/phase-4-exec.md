...
> **范围**：单 node 的执行内核（agent / script / set 三种叶子 kind）；**不做编排**（foreach 分批 / 拓扑 / 路由归 phase 5）。
...
**这一阶段不解决**（明确边界，见 §5）：
- 编排（拓扑 / 并行 / foreach / 路由 / 循环控制）—— phase 5
...
- ❌ **编排**（拓扑 / 并行 asyncio.gather / foreach 分批 / 路由 first-match-wins / 循环控制）—— phase 5
...
| 后续 | phase 4 提供 |
|---|---|
| phase 5 run | `make_executor(node) -> Executor`；`async executor.exec(node, ctx) -> AsyncIterator[Event]`；`RunContext(inputs, outputs, run_id)`。orchestrator 拿迭代器逐个 emit + 写 tape；拓扑/并行/路由/foreach 在 orchestrator |
...
## 不做的事（明确边界）
- ❌ 编排（拓扑/并行/foreach/路由/循环）—— phase 5
