"""03_model8_bn4relu.py —— kd-nas-demo KB 变体（原始 model8 + BN + 4层 + ReLU，不缩层）。

隔离「BatchNorm + ReLU」组合（保留原始 4 层不缩层），与主变体 ``00_model8_bn3relu`` 对照，
供 sweep 拆分 BN/ReLU vs 缩层各自的时延-精度贡献。架构同主变体（原始 model8），
唯一差异：``num_blocks`` 默认 4（原始 baseline 层数，不缩层）。可调旋钮仍含 num_blocks
（latency 超阈时 tune_latency 仍可缩到 min=2），但默认体现「不缩层」身份。
...
def build_model(**cfg) -> nn.Module:
    """实例化 BN + 4层 + ReLU 变体（cfg 取 num_blocks / embed_dim，缺省用 KNOBS.default）。"""
