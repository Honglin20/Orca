"""01_model8_bn3gelu.py —— kd-nas-demo KB 变体（原始 model8 + BN + 3层，保留 GELU）。

隔离「BatchNorm + 缩层」组合（不换激活），与主变体 ``00_model8_bn3relu`` 对照，
供 sweep 拆分 BN/缩层 vs ReLU 各自的时延-精度贡献。架构同主变体（原始 model8），
唯一差异：``act_type="gelu"``（保留原始 GELU）。
...
def build_model(**cfg) -> nn.Module:
    """实例化 BN + 3层 + GELU 变体（cfg 取 num_blocks / embed_dim，缺省用 KNOBS.default）。"""
