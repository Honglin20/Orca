...
1. **派一个子代理**执行当前节点：子代理 Read 节点指令文件（prompt 里给了路径）做完，
   ...
   派发词**只做指向**（让它 Read 该文件并严格照文件办），🔴 **不许转述 / 摘编 / 改写节点指令**
   （见【逐字传递铁律】）。
2. 子代理返回后，**先检查它的最终消息是不是 ask-user 哨兵**（见下【哨兵处理】）：
   - 是哨兵 → 走哨兵小循环：问用户 → 恢复**同一**子代理 → 拿真实产出。
   - 不是哨兵 → **一律**把最终消息**逐字节**作 `--output` 喂 `next`（一字不改地搬运，见
     【逐字传递铁律】；**包括子代理自报的失败**——引擎的哨兵检测 / schema 校验会判
     ``recoverable``，见下【产出不合 schema / 自报失败】）：
...
## 逐字传递铁律（anti-paraphrase）

产出链是两跳 LLM 链：子代理 → 你 → `orca next --output`。任何一跳转述都会污染产出——叙述回顾被
前置到 JSON 前、摘编丢字段、改写动字符——而引擎的 schema 校验只认原文，弹回就得重派，白费一次
派发。三条铁律：

1. **派发时逐字转发**：节点指令已由引擎落盘成文件（`prompt` 里给路径）。派发词只做一件事——让
   子代理 **Read 该文件并严格照文件办**。🔴 **你不许转述、摘编、改写或"补充"节点指令**——文件里
   的纪律条款（如「最终回复必须是命令 stdout 按字节回贴」）在你的复述里会丢失，子代理就会按你的
   转述版产出违约结果。
2. **回传时逐字节搬运**：子代理的最终消息作 `--output` 值时**逐字节搬运、一字不改**（单引号按
   【单引号转义】写法处理——那是 shell 编码，不改变值本身）。🔴 禁止加
   叙述回顾、禁止前置步骤总结、禁止包 code fence、禁止"整理格式"——这些前置散文正是 schema
   校验拒绝的污染。
3. **JSON 节点回传单行 JSON 原文**：节点的 output_schema 要求 JSON 时，子代理最终消息本身就是
   单行 JSON（该节点自己的输出纪律保证）。`--output` 的值必须是**那行 JSON 原文**，前后不许带
   任何字。
...
- [ ] 逐字传递：派发只指向节点指令文件（不转述/摘编）；`--output` 逐字节回传子代理最终消息（JSON 节点 = 单行 JSON 原文，前后零附加）
