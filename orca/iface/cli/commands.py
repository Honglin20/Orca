...
# ── resume 子命令（phase 11 §7 Checkpoint Resume）─────────────────────────────
...
def _resolve_tape_path(tape_or_run_id: str) -> Path:
    ...
    - 参数是已存在的文件路径 → 直接用。
    - 否则视为 run_id，查默认 ``runs/<run_id>.jsonl``（与 ``OrcaApp`` tape_path 约定一致）。
    """
    p = Path(tape_or_run_id)
    if p.is_file():
        return p
    # 当作 run_id：拼默认 runs 目录。
    return Path("runs") / f"{tape_or_run_id}.jsonl"
