...
def _format_stream_line(etype: str, data: dict) -> str:
    ...
    text = data.get("text") or data.get("tool") or data.get("reason") or ""
    if etype == "agent_tool_call":
        args = data.get("args")
        text = f"{data.get('tool', '?')}({_truncate_args(args)})"
    elif etype == "agent_tool_result":
        text = f"→ {_truncate(data.get('result'))}"
    elif etype == "foreach_started":
...
class NodeDetail(Static):
    ...
    def __init__(self) -> None:
        ...
        # node -> list[stream line]（按节点缓存；切回时显示历史）。
        self._stream_lines: dict[str, list[str]] = {}
    ...
    def append_event_stream(self, node: str, etype: str, data: dict) -> None:
        ...
        """
        self.append_stream(node, _format_stream_line(etype, data))
    ...
    def _refresh_stream(self) -> None:
        ...
        if node is None:
            static.update("（未选中节点）")
            return
        lines = self._stream_lines.get(node, [])
        if not lines:
