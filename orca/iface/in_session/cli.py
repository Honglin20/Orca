...
# 2026-08-26：``_read_workflow_name`` / ``_read_workflow_inputs`` /
# ``_resolve_artifacts_dir`` / ``_TAPE_HEAD_SCAN_LIMIT`` 已下沉 ``_artifacts.py``
# （in-session script 节点 spawn env 需要同一 project-scoped 派生，包内共享）；
# 私有名 re-import 保本模块既有调用点 / 测试 import 路径零改。
from orca.iface.in_session._artifacts import (
    TAPE_HEAD_SCAN_LIMIT as _TAPE_HEAD_SCAN_LIMIT,
    read_workflow_inputs as _read_workflow_inputs,
    read_workflow_name as _read_workflow_name,
    resolve_artifacts_dir as _resolve_artifacts_dir,
)
...
def _drive_protocol(run_id: str) -> str:
    ...
    **同一 next 命令本身**（不重派子代理 / 不重发 prompt；避免 advance_step 不持锁调用契约冲突）。
    ...
    return (
        ...
        "2. 子代理返回后，看它的产出：\n"
        "   - 产出是**合法 JSON**（节点指令里要求的输出格式）→ 把它作为 --output，**原样**执行"
        "下面这条命令（--run-id 是唯一句柄，必须用下面给定的值；tape 由 Orca 自己按 run_id 定位，不要传）：\n"
        f"   orca next --run-id {run_id} --output '<子代理的产出>'\n"
        "   - 产出**不是 JSON**、且含『请勿调用 orca next』字样（长任务节点的挂起标记，如"
        "训练未完成）→ **不要调 next**，节点保持执行中；等 CRON 定时唤醒你后，重读该节点的"
        "指令文件，重新派子代理检查状态（节点指令会说明挂起/完成规则）。\n"
...
# tape 头读取三 helper + 扫描上限原在此处，已下沉 ``_artifacts.py``（见模块头 re-import）。
