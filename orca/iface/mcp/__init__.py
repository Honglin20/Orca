...
单进程单 ``RunManager`` + ``FastMCP`` + 9 工具（Discovery 4 + Lifecycle 3 + History 2），
stdio transport。

v4 核心设计（2026-07-07 重写）：
  - **9 工具**：Discovery（list_workflows / describe_workflow / list_agents /
    get_agent_prompt）+ Lifecycle（start_workflow / get_task_status / cancel_task）+
    History（get_task_history / get_agent）。
  - **setup/execute 分相**：workflow schema 加 ``setup: list[AgentNode]`` 字段（可选）；
    execute phase agent 不配 ask_user/gate（compile validator 强制）。
  - **三重杠杆防跳过 setup**：list_workflows 标 ``has_setup`` / start_workflow 强校验
    ``setup_required`` / tool description 引导。
