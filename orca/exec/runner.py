...
SPEC §2.2 / §4.4。重写自 AgentHarness ``_cli_subprocess.py`` 的子进程模式
...
职责（SPEC §4.4）：
  ...
  - **进程组隔离**（phase-11 §2.1，推翻 phase-3 §2.5 旧决策）：spawn 用
    ``start_new_session=True``（POSIX）/ ``CREATE_NEW_PROCESS_GROUP``（Windows），
    让 claude/opencode spawn 的孙子进程（grep/bash/node）进同一进程组；cancel 时
    ``registry.kill_one`` 整组杀，**孙子进程不变孤儿**。
  ...
  - **超时 / 异常退出**：委托 ``registry.kill_one``（三段式 SIGTERM grace → SIGKILL →
    cleanup hooks，phase-11 §2.2）做进程组 cancel；async 路径经 ``asyncio.to_thread``
    包装避免阻塞 event loop。
...
设计规则：
  ...
  - ``registry`` 由调用方注入（DI，ADR §4.7）；默认 ``get_default_registry()``，
    测试用 ``process_local`` fixture 注入独立实例。

依赖单向：本模块依赖标准库 + ``orca/exec/registry``（同层 exec，纯进程基础设施）。
不依赖 schema/events/profiles/run/compile。
"""
...
# 超时后委托 registry.kill_one 的 grace 秒数（phase-11 §2.3，原 _KILL_GRACE_SECONDS）。
# 取 10s 上限（registry 内 _MAX_GRACE_SECONDS 校验），让慢 backend 有机会写完文件。
_KILL_GRACE_SECONDS = 10.0
# stream() 异常退出（外部 cancel / 中途退出）时，registry.kill_one 的 grace 秒；
# 正常完成路径 proc 已 exit（returncode 非 None），不走此分支。
...
class CLIRunner:
    ...
    def __init__(
        ...
        *,
        ...
        # production 用 ``get_default_registry()``；测试用 ``process_local`` fixture 注入。
        self._registry: ProcessRegistry = registry or get_default_registry()
        # registry.acquire 用：spawn 后立刻登记，附 backend/run_id/node_id 诊断信息。
        self._backend = backend
        self._run_id = run_id
        self._node_id = node_id
        # acquire 返回的 entry 引用（stream 结束时 release 用；None = 未 acquire）。
        self._entry: RegisteredProcess | None = None
    ...
    async def stream(self) -> AsyncIterator[str]:
        ...
        proc = await asyncio.create_subprocess_exec(
            *argv,
            ...
            # / Windows ``CREATE_NEW_PROCESS_GROUP``。让 claude/opencode spawn 的孙子进程
            # 进同一组；cancel 时 registry.kill_one 整组杀，孙子进程不变孤儿。
            **spawn_kwargs_for_process_group(),
        )
        # phase 11 §4.2：暴露 proc 句柄给 send_sigint（用户 Ctrl+G 中断用）。
        self._proc = proc
        # phase-11-process §1（铁律 1）：spawn 后立刻 registry.acquire 登记。
        # 不登记 = kill_one 找不到 entry = 兜底清理漏掉 = 孙子进程变孤儿。
        self._entry = self._registry.acquire(
            proc,
        ...
        try:
            ...
            # 跳过此分支。phase-11-process §2：委托 registry.kill_one 三段式进程组 cancel
            # （SIGTERM grace → SIGKILL → cleanup hooks），整组杀防孙子进程变孤儿。
            if proc.returncode is None:
                try:
                    await asyncio.to_thread(
                        self._registry.kill_one, proc.pid,
                        grace_seconds=_TERMINATE_GRACE_SECONDS,
                    )
                except Exception:
                    # registry 不应抛（kill_one 内部已吞各 stage 异常），但退出路径兜底
                    # 二次吞——绝不因 cleanup 失败把 stream() 的真实异常（如有）替换掉。
                    logger.warning(
                        "registry.kill_one 异常（stream finally 兜底吞，pid=%d）",
                        proc.pid, exc_info=True,
                    )
                # reap zombie：kill_one 已发 SIGKILL，proc.wait 不应再阻塞；2s 超时防御。
                try:
                    await asyncio.wait_for(proc.wait(), timeout=2.0)
                except (ProcessLookupError, asyncio.TimeoutError, asyncio.CancelledError):
                    pass
            # phase-11-process §1：release registry entry（无论自然退出还是 kill_one
            # 已 release，重复 release 幂等，防 entry 残留拖累 shutdown）。
            self._registry.release(proc.pid)
    ...
    async def _handle_timeout(self, proc: asyncio.subprocess.Process) -> None:
        """超时 → 委托 ``registry.kill_one`` 三段式进程组 cancel（phase-11-process §2.2）。

        原 SPEC §2.5「单进程 kill」决策已由 phase-11-process-lifecycle §2 推翻——
        killpg 整组杀才能兜住孙子进程（claude/opencode spawn 的 grep/bash/node）。
        grace_seconds = ``_KILL_GRACE_SECONDS``（10s 上限），让慢 backend 有机会写完
        文件 / 释放锁；grace 内未退 → SIGKILL 兜底。

        ``registry.kill_one`` 是同步函数（含 ``time.sleep`` poll），用 ``asyncio.to_thread``
        包装避免阻塞 event loop。返回后 ``await proc.wait()`` reap zombie。
        """
        self._result.timed_out = True
        logger.warning(
            "claude 子进程超时（timeout=%ss），委托 registry.kill_one 三段式 cancel",
            self._cfg.timeout,
        )
        try:
            await asyncio.to_thread(
            ...
            # registry 不应抛；二次兜底记 warning 让运维可见，但不替换 stream() 的真实异常。
            logger.warning(
                "registry.kill_one 异常（_handle_timeout 兜底吞，pid=%d）",
                proc.pid, exc_info=True,
            )
        # reap zombie：kill_one 已发 SIGKILL，proc.wait 不应再阻塞；加 2s 超时防御
        # kill_one 内部异常吞掉后 proc 仍存活的极端 race（不让 stream() 永久挂死）。
        try:
            await asyncio.wait_for(proc.wait(), timeout=2.0)
        except (ProcessLookupError, asyncio.TimeoutError, asyncio.CancelledError):
            pass  # 已 reap / 极端 race 超时 / 被 cancel：留给 atexit shutdown 兜底
    ...
    def _finalize(
        ...
        self._proc = None
        # phase-11-process：清 entry 引用（registry 内已 release，引用只供本对象诊断）。
        self._entry = None
