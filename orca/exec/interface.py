...
事件生命周期契约（所有 Executor 必须遵守，SPEC §4.2）::
    ...
    node_completed(node, session_id, data={output, elapsed})
      —— 或 ——
    node_failed(node, session_id, data={kind, error_type, message, phase})

phase-11 v2.1：``data.kind`` 是唯一分类权威（ErrorKind 值）；``error_type`` 保留为
读兼容期字段（旧 tape / 旧 reducer / 旧 widget）。session_id 由 executor 在 ``exec()``
入口生成（``uuid4().hex``），全程复用（铁律 5）。
"""
