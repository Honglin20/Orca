...
phase-13 §2：``make_executor`` 加可选 keyword ``runs_dir``，仅 agent 分支透传给
``ClaudeExecutor``（用于 spawn env 注入 ``ORCA_CHART_SOCK``）。None == 既有行为（向后兼容，
env 不注 chart 路由，script 端 render_chart 会 fail loud）。orchestrator 从
``self.bus.tape.path.parent`` 推导传入。
...
def make_executor(
    ...
    *,
    ...
    ``runs_dir``（phase-13 §2）：仅 agent 分支透传——ClaudeExecutor spawn 时把它和
    ``run_id`` 拼成 ``runs/<run_id>.sock`` 注入 ``ORCA_CHART_SOCK`` env。None == 不注
    （向后兼容，env 缺 → script 端 render_chart fail loud）。orchestrator 从
    ``self.bus.tape.path.parent`` 推导传入（同 tape 父目录）。
