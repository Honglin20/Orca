...
|---|---|
| **跑 / 校验 workflow** | |
| `orca run <yaml> [task] [-i key=value]... [--max-iter N] [--background]` | 跑 workflow（默认进 Textual TUI 看 DAG / 日志 / 答 gate；`--background` 后台跑） |
...
| `orca in-session doctor` | 入口链路自检（plugin 加载 / marker 派发 / CLI 可达） |
| `orca in-session serve` | 无头 CI / 长跑批处理 daemon |
| `orca in-session bootstrap` / `next` | 内部命令（由 plugin / hook 自动驱动，用户不直接敲） |
| **Web / MCP** | |
| `orca serve [--port 7428]` | 启动 Web UI（实时 DAG / 日志 / gate / tape 回放 / chart） |
...
| `/orca <sub>` | 用途 |
|---|---|
| `/orca doctor` | **入口链路自检**（plugin 加载 / marker 派发 / CLI 可达）——能回报告即 `/orca` 入口活着 |
...
```bash
orca serve                # → http://127.0.0.1:7428（首次用需先构建前端，见安装）
orca mcp                  # stdio JSON-RPC，供 Claude Code / opencode / Cursor 接入
orca mcp --with-web       # 同进程挂 Web UI，stdin EOF 后转 daemon
```

Web UI：左侧 run 列表 → **+New** 填 yaml 启动 → 实时 DAG / 日志；gate 弹窗作答；完成后 **⏮ Replay** 时间旅行回放。MCP 暴露 `start_workflow` / `get_task_status` / `resolve_gate` / `cancel_task` 四件套。
