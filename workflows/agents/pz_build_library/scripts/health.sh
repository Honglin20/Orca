#!/bin/bash
# health.sh —— 唤醒健康检查（BLD 进程活着时）：抓进度标记（epoch/step）/ 主指标值 / log 尾部。
...
if [ -f "runs/bld/progress.jsonl" ]; then
  grep -E ':[[:space:]]*-?(NaN|Infinity)' runs/bld/progress.jsonl 2>/dev/null | tail -2
else
