#!/usr/bin/env python3
"""eta.py —— 估时（信息用，落 .bld_eta.json，不决定 CRON 时机）。

从 log 抓当前 epoch/step 数 + run_bld.sh 的 --epochs / --max_steps → 每 epoch 耗时
（pidfile mtime / 当前 epoch）→ 剩余分钟。stdout 单行 JSON。
...
log = logs[-1] if logs else os.path.join(ad, "runs", "bld", "bld.attempt1.log")

total = None
sh = os.path.join(ad, "run_bld.sh")
...
per_epoch = None
pid_path = os.path.join(ad, "runs", "bld", ".bld_pid")
...
with open(os.path.join(ad, "runs", "bld", ".bld_eta.json"), "w", encoding="utf-8") as f:
    json.dump(out, f)
