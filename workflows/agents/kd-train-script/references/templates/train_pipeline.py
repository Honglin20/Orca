"""train_pipeline.py — KD-NAS unified training script (teacher + distill modes).

Generated by the ``kd-train-script`` agent.  Self-contained: copies the user's
loss / dataloader / optimizer logic inline (never imports the user's project),
loads teacher/student models **by path** via :mod:`importlib.util`, and runs on
a single device (no distributed data-parallel launch, no torchrun, no
architecture sampling — KD-NAS uses a ``ThreadPoolExecutor`` round-robin pool
at the workflow level, not inside the training script).
...
When the ``{{USER_TRAIN_MODULE}}`` placeholder is left unexpanded the script
runs on a placeholder fallback (dummy MSE loss + random re-iterable loader)
so it is smoke-testable standalone before the agent specialises it.
...
def _maybe_bootstrap_env(env_anchor: str) -> None:
    """BLK-5: best-effort ORCA env bootstrap from per-run artifacts anchor.
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--mode",
        required=True,
        choices=["teacher", "distill"],
        help="training mode: teacher (task_loss only) or distill (task_loss + KD)",
    )
    ...
    p.add_argument(
        "--env_anchor",
        default="",
        help="BLK-5: per-run $ORCA_ARTIFACTS_DIR anchor for env bootstrap",
    )
    return p.parse_args()
