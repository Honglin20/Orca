...
**auto-fixable**: no
**Section**: §2 Distributed Setup
**Check**: Standard `nn.Module` methods (`forward()` / `model(x)`, `parameters()`, `train()`, `eval()`, `state_dict()`, `zero_grad()`) are called directly on the DDP-wrapped model, with no unwrapping needed. Only custom supernet attributes (`search_space`, `get_active_subnet()`, `elastic_num_params`) access the inner module via `unwrap_model()`. `set_sample_config` is called via `set_sample_config_ddp()`.
...
- `--kd_warmup_start` defaults to ≈ 1/4 of the training budget (never `0`) per §8 KD Weight Warmup, so KD begins only after the shared weights have first converged under supervised loss.
- `--kd_warmup_length` defaults to a **nonzero** value ≈ 1/4 of the training budget per §8 KD Weight Warmup, not `0`.
**Verify**: If KD is enabled, check which KD loss function is used and verify it matches the output type (logits → `logits_kd_loss`, multi-label → `soft_bce_kd_loss`, continuous → `mse_kd_loss`, embedding → `cosine_kd_loss`). Verify shape guard exists. Verify `KDWeightScheduler` is constructed and its `get_weight()` result is used in the sandwich loop. Also verify that KD-related CLI args match the chosen loss: `--kd_temperature` should only exist when the loss accepts temperature (`logits_kd_loss`, `soft_bce_kd_loss`), not for `mse_kd_loss` or `cosine_kd_loss`. Check the argparse defaults for `--kd_warmup_start` and `--kd_warmup_length` are nonzero (≈ 1/4 of the training budget each) and roughly match the §8 guidance.
**Anti-pattern**: Using `mse_kd_loss` but exposing `--kd_temperature` and passing `temperature=args.kd_temperature` (MSE KD has no temperature parameter); adding unused KD CLI args to the launcher; using static `args.kd_weight` directly instead of `KDWeightScheduler`; defaulting `--kd_warmup_start` to `0` (KD starts at step 0 with an untrained teacher) or `--kd_warmup_length` to `0` (jumps KD to full weight immediately with an untrained teacher).
...
**Check**: The launcher exposes key training parameters as editable shell variables at the top: `DATA_DIR`, `OUTPUT_DIR`, training budget, `BATCH_SIZE`, `LR`, `NUM_WORKERS`, `EVAL_INTERVAL`, `SEED`, `MAX_GRAD_NORM`, `SANDWICH_N_RANDOM`, `AMP`. `NUM_WORKERS` defaults to `0` (DataLoader Launch Hygiene, item 36). `AMP` defaults to `false` (single-device default). When KD is enabled, also: `KD_WEIGHT`, `KD_WARMUP_START`, `KD_WARMUP_LENGTH`.
**Verify**: Read the editable variables section of `run_train_supernet.sh`.
**Anti-pattern**: Hardcoding values in the launch command instead of using variables; missing key variables; including `NNODES`/`NPROC_PER_NODE`/`MASTER_PORT` (v2 default is single-process, no torchrun).
...
**Verify**: grep for `python3 train_supernet.py` in `run_train_supernet.sh`. Verify `torchrun` is NOT present in the default launcher.
