#!/usr/bin/env python3
"""emit_result.py —— psu_retrain 最终 JSON（**唯一产出节点 JSON 的时刻**）。

status 推导优先级（互斥，先命中先定）：
1. retrain.py 或 run_retrain.sh 不存在 → failed（上游 psu_retrain_script 未产出脚本——前置错误）
...
fidelity_retriggered 恒 false（执行节点不重触 fidelity——归上游 psu_retrain_script）。
...
ckpt_marker = os.path.join(ad, ".psu_retrain_ckpt_resolved.txt")
...
if not (os.path.exists(retrain_py) and os.path.getsize(retrain_py) > 0
    ...
    if log_tail:
        prev = read_text(os.path.join(ad, ".psu_retrain_assessment.txt"), "")
        with open(os.path.join(ad, ".psu_retrain_assessment.txt"), "w", encoding="utf-8") as fh:
            fh.write((prev + "\n" if prev else "") + "last_error:\n" + log_tail)

healed_files = read_lines(os.path.join(ad, ".psu_retrain_exec_healed.txt"))
# 执行节点不重触 fidelity（归上游 psu_retrain_script）；此字段恒 false。
fidelity_retriggered = False
assessment = read_text(os.path.join(ad, ".psu_retrain_assessment.txt"), "no assessment recorded")
