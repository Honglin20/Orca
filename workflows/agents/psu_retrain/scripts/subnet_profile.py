...
Reads ``.selected_arch.json`` (written by select_architecture.py via psu_run_search /
psu_select), materializes the active subnet via the sibling ``supernet.py``'s
...
def main() -> int:
    ...
    # Fail-soft wrapper.
    try:
        subnet, num_classes_used, in_channels_used, input_hw = _materialize(ad, selected_path)
    ...
    total_macs = _compute_macs(subnet, in_channels_used, input_hw)
    per_layer = _per_layer_rows(subnet, in_channels_used, input_hw)
...
def _materialize(ad: Path, selected_path: Path):
    ...
    Returns ``(subnet_module, num_classes_used, in_channels_used, input_hw)``. Raises ``_FailSoft``
    ...
    num_classes, in_channels, input_hw = _read_project_dims(ad)
    ...
    subnet.eval()
    return subnet, num_classes, in_channels, input_hw


def _build_arch_config(ArchConfigCls, selected_arch: dict):
    """Construct an ArchConfig from the selected_arch dict (per-slot choice layout).

    The selected architecture is choice-only: one branch name per transformer
    layer slot (depth / num_heads / ffn_dim are pinned to the original model's
    measured values and never appear here). JSON round-trips the per-slot choice
    vector as a list of branch names; convert such vectors to tuples so
    tuple-typed / dataclass ArchConfig fields accept them. Every other field
    passes through as a keyword arg untouched — if construction raises (unknown
    kwarg), the caller's fail-soft wrapper catches it and writes stderr.
    """
    kwargs = {}
    for key, val in selected_arch.items():
        if isinstance(val, list) and val and all(isinstance(item, str) for item in val):
            kwargs[key] = tuple(val)  # per-slot branch-choice vector: list -> tuple
        else:
            kwargs[key] = val
    return ArchConfigCls(**kwargs)


def _read_project_dims(ad: Path) -> tuple[int, int, tuple[int, int]]:
    """Read ``num_classes`` / ``in_channels`` / ``input_size`` from the manifest.

    Priority: project_manifest.md fact lines. A fact missing from the manifest falls
    back to an MNIST-like placeholder AND is flagged on stderr as ``assumed`` —
    wrong dims mean wrong total_params / MACs / head shape, which must be
    observable rather than silently wrong.
    """
    nc: int | None = None
    ic: int | None = None
    ...
    if manifest.is_file():
        ...
        for line in text.splitlines():
            low = line.lower()
            if nc is None and "num_classes" in low:
                val = _int_after_colon(line)
                if val is not None:
                    nc = val
            if ic is None and ("in_channels" in low or "input_channels" in low):
                val = _int_after_colon(line)
                if val is not None:
                    ic = val
            if hw is None and ("input_size" in low or "input shape" in low):
                nums = _ints_after_colon(line)
                if len(nums) == 1:
                    hw = (nums[0], nums[0])
                elif len(nums) == 2:
                    hw = (nums[0], nums[1])
                    ...
                    if ic is None:
                        ic = nums[0]
    if nc is None:
        sys.stderr.write("[subnet_profile] num_classes not found in project_manifest.md; assumed 10\n")
        nc = 10
    if ic is None:
        sys.stderr.write("[subnet_profile] in_channels not found in project_manifest.md; assumed 1\n")
        ic = 1
    if hw is None:
        sys.stderr.write("[subnet_profile] input_size not found in project_manifest.md; assumed 28x28\n")
        hw = (28, 28)
    return nc, ic, hw
...
def _ints_after_colon(line: str) -> list[int]:
    """Extract every integer in the value part after the first ``:`` / ``=`` on a line."""
    import re
...
def _compute_macs(subnet, in_channels: int, input_hw: tuple[int, int]):
    ...
    try:
        # Dummy input built from the manifest's input_size (flagged `assumed` on stderr
        # by _read_project_dims when the manifest lacks the fact). The MACs count's
        # exact spatial dims don't affect the structural comparison (only weights
        # matter proportionally); the manifest dims keep the representative realistic.
...
def _per_layer_rows(subnet, in_channels: int, input_hw: tuple[int, int]) -> list[dict[str, Any]]:
    ...
    out_shapes = _shapes_via_forward(subnet, in_channels, input_hw)
...
def _shapes_via_forward(subnet, in_channels: int, input_hw: tuple[int, int]) -> dict[str, Any]:
