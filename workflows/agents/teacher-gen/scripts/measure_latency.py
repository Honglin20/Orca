"""measure_latency.py —— teacher-gen 契约默认 cfg 的 latency 测量（自包含，fail loud）。

把「teacher 文件的 `__main__` 跑一次 = 正确性 + latency」中的 latency 部分抽成 helper，
供 teacher-gen agent / teacher 文件 `__main__` 复用。

**与 `model-flatten/scripts/measure_latency.py` 字节对齐**（teacher-gen 复用 flatten 的 latency
测量实现，保持各 folder-agent 独立——`$ORCA_AGENT_RESOURCES` 锚定时 teacher-gen 自带 helper，
不跨 agent 引用）。同步由 ``tests/workflows/test_teacher_gen.py::test_measure_latency_copy_sync_with_flatten``
守门（防漂移）。未来可抽到共享位置（如 `_kd_scripts/`，见 release note Open Questions）。

与 `validate_teacher.py` 同目录（`teacher-gen/scripts/`），**不 import `_kd_scripts` /
`_struct_scripts`**（teacher-gen 保 standalone，与 flatten 同款自包含原则；ONNX 导出 inline
实现，不依赖 `export_onnx.py`）。
...
def _emit(key: str, value: Any) -> None:
    """stdout ``KEY: value`` 行（teacher-gen agent bash awk 解析）。value 非 str → JSON 串。"""
...
def _measure_cpu_fallback(
    ...
    latency_provider 时不应走到这里；teacher-gen agent 自身容错为通用性）。
    """
    import numpy as np
    import onnxruntime as ort
