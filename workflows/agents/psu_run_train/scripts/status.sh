#!/bin/bash
# status.sh —— psu_run_train 状态确定性判定（gate + 完成 + 存活 三合一，agent.md Step 1）。
# 输出一行判定（互斥）：
#   TRAIN_SCRIPT_MISSING  run_train_supernet.sh 不存在（上游生成失败 → fail loud 非 skip）
...
if [ ! -f run_train_supernet.sh ]; then
  printf "run_train_supernet.sh absent: upstream psu_train_script did not produce it (fail loud)." \
    > .psu_run_train_assessment.txt
  echo "TRAIN_SCRIPT_MISSING"
  exit 0
fi
...
" "$CKPT" 2>/dev/null | grep -q CKPT_VALID; then
  ...
  printf '%s' "$CKPT" > .psu_run_train_ckpt_resolved.txt
  printf 'training completed (rc=0, process exited, ckpt valid): %s' "$CKPT" > .psu_run_train_assessment.txt
