...
rm -f .psu_run_train_healed.txt .psu_run_train_fidelity.flag .psu_run_train_assessment.txt .psu_run_train_ckpt_resolved.txt runs/train/progress.jsonl runs/train/.train_rc
...
# --title 省略：watcher 自推导 "<label> (attempt N)"（N 读本 attempt 写入的 .train_attempt），
# 与 agent 完成路径补跑同推导 → 同 label+attempt 同 title（dedup 替换语义）。
setsid nohup bash -c 'python3 "$ORCA_AGENT_RESOURCES/scripts/progress_watcher.py" --progress "runs/train/progress.jsonl" --done-marker "runs/train/.train_rc" --label "puzzle-supernet/train" >/dev/null 2>&1 & bash run_train_supernet.sh > "runs/train/train.attempt'"$N"'.log" 2>&1; echo $? > runs/train/.train_rc' >/dev/null 2>&1 &
