...
* The exported subnet must preserve the same fixed non-searchable modules (`stem`, `stage_downsample`, task head) and the same searchable stage structure as the active supernet path. Fixed modules that carry parameters, and any buffer or parameter registered directly on the supernet or a fixed module (`register_buffer`, `register_parameter`, or a bare `nn.Parameter` attribute), should be deep-copied (`copy.deepcopy`) into the exported subnet.
