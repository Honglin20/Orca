...
本节点 Tier B 项（"读用户代码可得"的结构片段，缺失走哨兵而非造假）——**仅 Step 6 抽取用户 train.py 时触发**：

- **dataloader 构造**（用户 train.py 里 `DataLoader(...)` 的构造片段 / `def load_train` dotted-path）
- **loss 函数**（`criterion = ...` 或 `def <X>_loss` 的 callable 引用）
- **optimizer / scheduler**（`optim.<Opt>(...)` / `scheduler = ...` 的构造片段）
- **train-loop 骨架**（`for epoch in ...` / `for batch in <loader>` 的循环结构）

`grep -nE` 用户 train.py 找上述结构：

- **全找到** → 套 `_kd_scripts/train_adapter_template.py` 模板填充 placeholder → 落 `$TRAIN_KD_PATH`。
- **任一抽不出** → **不要**造假（编造默认 DataLoader / 写空 loss / 假 optimizer / 静默套模板跳过缺失项），
  以**最终消息**返回轻量哨兵 JSON（且仅此）：

  ```json
  {"_orca_ask_user": "<一句话问题，如 '你 train.py 里训练 loss 的定义在哪？请贴 callable 名或代码片段'>",
   "options": ["<候选 1，如 'criterion = nn.MSELoss()'>", "<候选 2>"],
   "context": "<已 grep 过 train.py 的哪些模式、看到了什么、缺的是哪一项>",
...
- 你**会被恢复**（不是重跑）——主 session 收到哨兵会用 SendMessage / Task(task_id) 把用户答案追加给你。
  ...
  profile_report 等已落盘的产物保留，只续做 Step 6 的 train_kd.py 生成）。
- 用户也答不出（连续多次「不知道」） → 返回 `{"_status":"fail_loud","reason":"<缺什么>"}`。

> 范围外：Step 1 的 `build_fn` / `dummy_input` / `project_root` 探测**不**走哨兵——它们有「低置信」标注
> fallback（不阻塞），且属结构 introspection 而非训练必填项。Step 3/4/5 的脚本非零退出是确定性 fail loud
> （脚本 bug / teacher 训失败 / ONNX 导不出），不是「读用户代码无果」，不走哨兵。
