...
- 校准 loader dotted-path: `{{ inputs.calib_data_ref }}`（duquantpp scheme 校准用；空则假随机）
- 训练 loader dotted-path: `{{ inputs.train_data_ref }}`（空则复用 calib 做最小 smoke QAT；**真实 QAT 恢复需真实训练数据**）
- 评估 loader dotted-path: `{{ inputs.eval_data_ref }}`（空则复用 train）
- eval_fn dotted-path: `{{ inputs.eval_fn_ref }}`（空则 teacher-student mse；训练 loss 始终用 teacher-student mse，label-free distillation-style QAT）
...
2. **生成 `<output_dir>/adapter.py`**：读 `{{ inputs.model_path }}` 理解模型 forward 签名与 batch 形态，写一个适配模块，暴露：
   ...
   - `get_calib_loader() -> DataLoader`：校准 loader（duquantpp 用）。优先按 `{{ inputs.calib_data_ref }}` import；为空则假随机。
   - `get_train_loader() -> DataLoader`：训练 loader。优先按 `{{ inputs.train_data_ref }}` import；为空则**复用 `get_calib_loader()`**（最小 smoke）。
   - `get_eval_loader() -> DataLoader`：评估 loader。优先按 `{{ inputs.eval_data_ref }}` import；为空则复用 train loader。
   - `forward_fn(module, batch) -> Tensor`：按模型 forward 解包 batch。
   - `get_eval_fn()` / `get_metric_spec()`（**仅** `{{ inputs.eval_fn_ref }}` 非空时实现）：业务评估函数（签名 `eval_fn(student_model) -> {"<metric>": float}`）+ `{primary_metric, higher_is_better}`。空则不生成（默认 teacher-student mse / lower-is-better）。
