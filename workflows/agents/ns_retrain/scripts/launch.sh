...
rm -f .ns_retrain_healed.txt .ns_retrain_assessment.txt .ns_retrain_ckpt_resolved.txt
...
# 绝不碰训练 rc）——边重训边推实时 loss 曲线到前端。
setsid nohup bash -c 'python3 "$ORCA_AGENT_RESOURCES/scripts/live_loss_watcher.py" --log "runs/retrain/retrain.attempt'"$N"'.log" --done-marker "runs/retrain/.retrain_rc" --label "nas-supernet/retrain" --title "Retrain Loss (attempt '"$N"')" >/dev/null 2>&1 & bash run_retrain.sh > "runs/retrain/retrain.attempt'"$N"'.log" 2>&1; echo $? > runs/retrain/.retrain_rc' >/dev/null 2>&1 &
