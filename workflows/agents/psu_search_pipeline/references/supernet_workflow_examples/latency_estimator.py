"""Online latency estimator for the generated supernet.
...
class LatencyEstimator:
    ...
    def get_latency(self, arch_config: ArchConfig) -> float:
        ...
        # See "Handling Non-Searchable Model Logic" in the workflow doc. If
        # this subnet had a data-dependent convergence loop, freeze it here
        # via a nested function measuring a single iteration, e.g.:
        #   subnet.to(self.device)  # callables must place the module
        #                           # themselves; measure_module_latency
        #                           # only does this for a plain nn.Module.
        #   def _frozen_forward(y):
        #       h = subnet.initialize_solution(y)
        #       return subnet.latent_space_forward(y, h)[0]
        # ...and pass `_frozen_forward` to measure_module_latency below
        # instead of `subnet`.
