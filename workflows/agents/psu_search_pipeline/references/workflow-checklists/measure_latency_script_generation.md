...
**Anti-pattern**: Any CLI args beyond the five specified; missing assertion on non-negative latency.

### [MAJOR] 8. Dynamic Non-Searchable Logic Frozen For Measurement
**auto-fixable**: no
**Section**: Handling Non-Searchable Model Logic
**Check**: If the exported subnet class carries dynamic Non-Searchable Model Logic (e.g. a fixed-point/convergence loop whose iteration count depends on the random dummy input, a `self.training`-gated safeguard, or other input/threshold-dependent control flow reachable from `forward()`), `get_latency()` freezes it to a fixed, deterministic path before measuring, via a model-specific helper called after `get_active_subnet()`/`subnet.eval()` and before dummy-input construction. When the supernet has no such dynamic logic, no freeze step is required.
**Verify**: Cross-reference the exported subnet class in `supernet.py` for iterative/fixed-point loops with data-dependent termination, `self.training` branches with side effects, or other reachable randomness. If present, confirm `get_latency()` in `latency_estimator.py` pins the relevant attributes (or wraps the call in a prepared callable) before calling `measure_module_latency`.
**Anti-pattern**: Measuring latency directly on a subnet whose `forward()` contains a data-dependent convergence loop or training-gated safeguard without freezing it first, causing latency to vary between calls for the same architecture.

### [MAJOR] 9. API Consistency With Supernet (Cross-Reference)
**auto-fixable**: no
