...
从 log 抓当前 epoch 数 + run_retrain.sh 的 --epochs（或 gkd_retrain.py 的 argparse 默认）
→ 每 epoch 耗时（pidfile mtime / 当前 epoch）→ 剩余分钟。stdout 单行 JSON。
...
if os.path.exists(sh):
    ...
    if m:
        ...
        if m:
            total = int(m.group(1))
if total is None:
    # 回落：扫 gkd_retrain.py 的 argparse default
    for cand in ("gkd_retrain.py",):
