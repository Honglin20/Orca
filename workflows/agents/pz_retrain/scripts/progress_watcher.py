...
用法（launch.sh 内、训练脚本前启动）：
    python3 progress_watcher.py --progress "runs/retrain/progress.jsonl" \
        --done-marker "runs/retrain/.retrain_rc" \
        --label "puzzle/retrain" --title "Retrain Metrics (attempt 1)" \
...
def main() -> int:
    ...
    ap.add_argument("--done-marker", required=True,
                    help="本次 attempt 完成 marker（launch.sh wrapper 末尾写 .retrain_rc）")
