#!/usr/bin/env bash
# reuse_check.sh — psu_run_search Step 0 reuse gate.
...
" "$RESULTS" 2>/dev/null | grep -q RESULTS_VALID; then
    ...
    rm -f .psu_run_search_healed.txt .psu_run_search_fidelity.flag
    printf 'reused existing search_results.jsonl: %s' "$RESULTS" > .psu_run_search_assessment.txt
    # reuse also appends anchors FIRST (so the charts below include the all-original
    # baseline anchor) and then pushes the search 3 charts, otherwise the frontend
    # never sees the Pareto / search table / latency distribution.
    python3 "$SCRIPTS_DIR/append_anchor_candidates.py" --artifacts-dir "$ARTIFACTS_DIR" >/dev/null 2>&1 || true
    ...
    echo "REUSE: search_results.jsonl exists and meets the bar → skip search redo, appended anchors + pushed 3 charts → proceed to Step 2.8 select → Step 3"
    exit 0
  fi
fi
