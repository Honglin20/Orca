# Hierarchical Transformer Supernet Readiness

Rules in this file apply when the model is classified as a hierarchical multi-stage 2D vision Transformer: fixed macro-stages with token-merging, pooling, or spatial-downsampling transitions between them.

> **Scope**: This file targets **2D vision** (images) only. `TransformerStageDownsample2d`, window partitioning, and the referenced NAS prebuilt blocks are all 2D-specific. Other spatial domains (3D volumetric, video) have no matching readiness rule here and are not currently supported by this skill.
...
**Implementation**
1. Change each block's `forward()` signature from `(B, L, C)` to `(B, H, W, C)` (BHWC). The channel dimension `C` stays on the last axis, which is the same semantic as BLC, just with spatial structure exposed.
...
3. `LayerNorm` and `Linear` operate directly on the last dimension `C`, so they work identically on both `(B, H, W, C)` and `(B, L, C)` shapes without any code change.
