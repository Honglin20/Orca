---
description: Generate retrain.py + finetune.py + run_retrain.sh for the puzzle-supernet pipeline with the fixed finetune-from-supernet strategy — materialize the selected subnet, strict-load its branch weights from the supernet ckpt per the selected choices, and finetune with the same frozen pretrained teacher KD (freeze continuation). Port the user's data/eval logic and close the loop via fidelity/workflow/memory verifiers.
tools: [bash, read, write, edit, glob, grep, task]
---
# psu_retrain_script

You are the **subnet retrain script generation** folder-agent of the puzzle-supernet pipeline. The retrain strategy
is a **constant `finetune-from-supernet`**: materialize the selected subnet via `get_active_subnet`, strict-load the
subnet's branch weights from the supernet checkpoint (`search_config.yaml` → `supernet_ckpt_path`) per the selected
per-slot choices, and finetune it with the **same frozen pretrained-teacher KD** used during supernet training
(freeze continuation: only the selected path's variant-branch params are trainable). Read `$ORCA_ARTIFACTS_DIR`
(`supernet_summary.md` + `project_manifest.md` + `search_config.yaml` + `supernet.py` + `load_pretrained.py` +
`train_supernet.py`) plus the user's original project (`{{ inputs.project_root }}`), generate `retrain.py` +
`finetune.py` + `run_retrain.sh` + any necessary helpers, then close the loop via the fidelity/workflow/memory
verifiers. A missing supernet ckpt or teacher ckpt → fail loud (output_schema `error` field) — there is **no
from-scratch fallback**. The downstream `psu_retrain` node executes the generated launcher to obtain the final
subnet weights. This node **does not run training**.
...
- `$ORCA_ARTIFACTS_DIR`: must contain `supernet_summary.md`, `project_manifest.md`, `supernet.py`,
  `search_config.yaml`, `load_pretrained.py` (the teacher / weight-inheritance loader generated at flatten), and
  `train_supernet.py` (the KD recipe + freeze groups to mirror) (see **Pipeline Memory**). Any of the mandatory
  files missing → fail loud (output_schema `error` field stating which one is missing), no silent defaults.
- `{{ inputs.project_root }}`: the original PyTorch project root. If absent → read it from the **Source Project**
  section of `supernet_summary.md`.
- `{{ inputs.pretrained_ckpt }}`: the pretrained original-model checkpoint — the frozen teacher's weight source,
  loaded through the same `load_pretrained.py` the supernet training used. Missing/unloadable → fail loud.
- `{{ psu_run_search.output.selected_arch }}`: the architecture selected upstream (Jinja-rendered per-slot choice
  dict — one branch name per transformer layer slot). The architecture source for generating `retrain.py`. If
  absent/null → fail loud (nothing to retrain).

## Pipeline Memory

Cross-session documents in `$ORCA_ARTIFACTS_DIR` (shared with upstream nodes):

- **`supernet_summary.md`**: NAS pipeline status. This node READS the KD recipe facts recorded by `psu_train_script`
  (loss composition: hidden cosine + logits KL, loss weights, teacher = pretrained original model, freeze groups) —
  the retrain KD mirrors them exactly — and the **Evaluation Paradigm** section (informational context only). This
  node may append the retrain artifacts to the **Generated Artifacts** list; it must NOT rewrite paradigm / KD
  recipe facts (those belong to `psu_train_script`).
...
- **Final-ckpt contract (cross-node, shared with `psu_retrain`)**: the final best subnet checkpoint produced by the
  generated `retrain.py` is always written to `$ORCA_ARTIFACTS_DIR/runs/retrain/retrain_best.pth` (a path relative
  to `$ORCA_ARTIFACTS_DIR`). This is the contractual path consumed by the downstream `psu_retrain` node's
...
- **training loss**: NOT ported from the user's project — it is the workflow-fixed KD recipe (frozen pretrained
  teacher + hidden cosine + logits KL), mirrored verbatim from `train_supernet.py` (a declared designed deviation,
  see below)
...
subnet-ization**: materializing the one selected fixed subnet, strict-loading its branch weights from the supernet
ckpt per the selected per-slot choices, replacing the training loss with the frozen-teacher KD recipe (mirrored from
`train_supernet.py`), continuing the freeze grouping (only the selected path's variant-branch params are trainable),
and budget compression (epochs/batches reduction + scheduler rescale).

**Forbidden substitutions**: do not introduce proxies the user never declared — including switching optimizer
classes on your own (e.g. AdamW→SGD), changing the KD loss composition (hidden cosine + logits KL and their weights
must mirror `train_supernet.py`), changing metric names/directions/transforms, loss↔acc swaps, arbitrarily negating /
inverting transformations, unfreezing original branches or non-slot modules, putting the frozen teacher's parameters
into the optimizer, or bypassing the freeze continuation.
...
> `finetune.py` + `run_retrain.sh` (all land in `$ORCA_ARTIFACTS_DIR/`). This step
...
- If qualifying (all three artifacts present + `retrain.py` syntax OK + `run_retrain.sh` references it) → skip Steps 1-3
  and emit per the existing output_schema: read `retrain_strategy` / `retrain_script_path` / `retrain_py_path` as
  real paths from disk + `error=""` + `generated_artifacts` listing the existing artifacts. `retrain_strategy` is the
  constant `finetune-from-supernet` (the reuse check requires `finetune.py` present — a file set without it is an
  incomplete product, not a different strategy). Do not rewrite the reused artifacts — their mtimes must stay
  earlier than this run's start so reuse is mechanically checkable.
...
1. **Read the KD recipe facts:** read `$ORCA_ARTIFACTS_DIR/supernet_summary.md`. Extract the KD recipe recorded by
   `psu_train_script` (loss composition: hidden cosine + logits KL, loss weights, teacher = pretrained original
   model, freeze groups) — the retrain KD mirrors it exactly — and the **Evaluation Paradigm** (informational only).
...
3. **Read the supernet + ckpt path:** read `$ORCA_ARTIFACTS_DIR/supernet.py` for the `SearchSpace` / `ArchConfig` /
   `SuperNet` API (`set_sample_config` / `get_active_subnet` materialization key contract). Read
   `$ORCA_ARTIFACTS_DIR/search_config.yaml` for the `supernet_ckpt_path` (contractual default
   `runs/train/supernet_best.pth`). Read `$ORCA_ARTIFACTS_DIR/evaluator.py` for the subnet-extraction and
   weight-loading logic actually used during search. Read `$ORCA_ARTIFACTS_DIR/train_supernet.py` for the KD loss
   implementation, teacher construction, freeze grouping, and checkpoint conventions to mirror. Read
   `$ORCA_ARTIFACTS_DIR/load_pretrained.py` (generated at flatten) — the teacher instance and original-branch weight
   checks load through this same script.
...
### Step 2: Verify Strategy Prerequisites + Generate Artifacts

#### Strategy Prerequisites (deterministic, do this first)

The retrain strategy is **fixed**: `finetune-from-supernet`. There is no from-scratch fallback and no LLM judgment
call — a from-scratch retrain would discard both the pretrained weight inheritance and the KD distillation premise.
Verify the strategy's two prerequisites deterministically; a missing prerequisite → **fail loud**, never a silent
downgrade:

1. supernet ckpt = the `supernet_ckpt_path` from `search_config.yaml` (contractual default
   `runs/train/supernet_best.pth`); the file must exist and be non-empty (`[ -s "$ckpt_path" ]`).
2. teacher ckpt = `{{ inputs.pretrained_ckpt }}`; the file must exist and be non-empty.

- Both present → proceed; write a one-line `strategy_reason` evidence record (e.g.
  `finetune-from-supernet: supernet ckpt runs/train/supernet_best.pth present, teacher ckpt <path> present`).
- Either missing → fail loud: emit the output JSON with `error` stating which prerequisite is absent and empty
  artifact paths. Do **not** generate a degraded training script instead.
...
This path produces `retrain.py` + `finetune.py` + `run_retrain.sh` + necessary helpers
(ported by you or a porter). The selected architecture is the Jinja-rendered `{{ psu_run_search.output.selected_arch }}`
(a per-slot choice dict — one branch name per transformer layer slot). After generation, first run the fidelity audit
loop, then the workflow compliance loop.
...
- The intended behavior of the generated `retrain.py`: how it is designed to deviate from the original project. Fill
  ...
  ```
  - Training loss = the fixed KD recipe (frozen pretrained teacher, hidden cosine + logits KL) mirrored verbatim from train_supernet.py — a declared designed deviation from the user's original supervised loss; changing the KD loss composition or diverging from the supernet-training implementation is forbidden. Optimizer / scheduler / data flow / control flow are taken verbatim from the original project (see the manifest's "user measurement checklist"), with only subnet-ization changes; replacing the optimizer class or changing metric names/directions/transformations is forbidden.
  - Weight inheritance: the selected subnet is materialized via get_active_subnet and its branch weights are strict-loaded from <supernet ckpt path> per the selected per-slot choices; the freeze grouping continues (only the selected path's variant-branch params are trainable; original branches and non-slot modules stay frozen).
  - Teacher = frozen pretrained original-model instance built via load_pretrained.py from <pretrained_ckpt path> (requires_grad_(False) + eval + no_grad forward), identical to the supernet-training teacher.
  ...
  - Retrain strategy: finetune-from-supernet (constant; reason: <strategy_reason>). The single selected subnet is finetuned to convergence.
...
- **Artifacts** (verifier may modify): `retrain.py`, `finetune.py`, `run_retrain.sh` + any generated helpers (`finetune.py` is mandatory, never conditional).
...
1. **Append to `supernet_summary.md` Generated Artifacts:** open `$ORCA_ARTIFACTS_DIR/supernet_summary.md` and append
   ...
   evaluation-paradigm / KD recipe sections (those belong to `psu_train_script`).
...
- **Hardened script gate (deterministic, must run after generation)**:
  ...
  || { echo "FAIL" >&2; exit 1; }
  ```
  Checks py_compile (`retrain.py` + `finetune.py`, both mandatory) + conditional DDP (`is_distributed()` guard + wrap) +
  guarded `sync_random_seed` + launcher hygiene (delegated to `check_launcher.sh`: no torchrun + AMP=false +
  NUM_WORKERS=0 + `python3 retrain.py` entry) + progress.jsonl write contract (step-level granularity:
  `--progress-every` or an equivalent step-modulo write) + deterministic terminal metric lines
  (`[eval] unit <N> <metric> <value>` / `done best <metric> <value>`) + frozen-teacher KD / weight-inheritance static gates. On failure → fix-loop.

- **User-paradigm self-check (deterministic, must run after generation)**: grep `retrain.py` / `finetune.py` for the
  optimizer construction + KD loss call tokens — the optimizer class name must match the optimizer recorded in
  `project_manifest.md`'s Training And Evaluation section (if the manifest records `Adam`/`AdamW`, undeclared
  substitutions like `SGD` must not appear); the KD loss helpers (hidden cosine + logits KL + their weights) must
  mirror `train_supernet.py`'s implementation (same helper names / same composition). **Evaluation-path
  ...
  > mechanically comparable optimizer/KD-loss tokens and the evaluation-entry/metric tokens.
...
- For generated remote-execution scripts such as `retrain.py` / `run_retrain.sh`, follow the specific validation
  ...
  locally — full execution is the downstream `psu_retrain` node's job.
...
```json
{
  ...
  "retrain_strategy": "<finetune-from-supernet>",
  "strategy_reason": "<one-line evidence: supernet ckpt + teacher/pretrained ckpt presence>",
...
- `retrain_strategy`: the constant `finetune-from-supernet` (materialize the selected subnet + strict branch-weight
  load from the supernet ckpt + frozen-teacher KD finetune, freeze continuation). No alternative value exists.
- `strategy_reason`: one line of evidence (supernet ckpt + teacher/pretrained ckpt presence).
...
- `error`: on fail loud, state the root cause (e.g. `$ORCA_ARTIFACTS_DIR` is missing a mandatory upstream artifact —
  state which one; the supernet ckpt / teacher ckpt prerequisite is absent; or `selected_arch` is null). Empty
  string on success.
