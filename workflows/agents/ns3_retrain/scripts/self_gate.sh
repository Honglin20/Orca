#!/usr/bin/env bash
# self_gate.sh — ns3_retrain Step 0 self-gate on upstream-generated scripts.
...
if [ ! -s retrain.py ] || [ ! -s run_retrain.sh ]; then
  printf "%s" "upstream ns3_retrain_script did not produce retrain.py/run_retrain.sh" > "$ARTIFACTS_DIR/.ns_retrain_assessment.txt"
