---
description: nas-supernet-v2 搜索执行 + select 合并 agent（folder-agent）。运行上游 ns2_search_pipeline 生成的 run_search_supernet.sh → 推搜索 3 图 → 运行 select_architecture.py 选架构 → emit 含 select 5 字段的 JSON。无上限自愈直到 search_results.jsonl ≥1（rc=0）。select 崩/无候选时 emit falsy select 字段（禁 node_failed）→ ns2_report 归因 select_failed。触碰搜索/evaluator 逻辑类目 → 重触 project-fidelity-verifier（point-to-file 协议）。output_schema 双层强制单行 JSON。
...
上游 `ns2_search_pipeline` 已在 `$ORCA_ARTIFACTS_DIR` 生成搜索脚本 `run_search_supernet.sh`
...
- `$ORCA_ARTIFACTS_DIR`（orca spawn 注入）= 本 run 的 artifacts 目录，上游
  ns2_search_pipeline 落脚本处，跨节点共享；`search_results.jsonl` 权威产物落此处。
...
- 达标（`search_results.jsonl` ≥1 行合法 JSON）→ 跳过 Step 1 / Step 2 的搜索重做，但**仍走 Step 2.7 推 3 图 +
  Step 2.8 select**（reuse 也要产 .selected_arch.json marker 供 ns2_report 读终态），再进 Step 3 emit
...
- **status 枚举不动**：reused 走 `executed`（成功路径同一 status，ns2_run_search 路由守卫读 executed
  不误判）。
...
- 常见根因判定：
  - 缺 supernet ckpt → 回看 ns2_run_train output。若 ns2_run_train `status=skipped` / `failed`，ckpt 注定
...
1. 调 host 内置通用 subagent（point-to-file 协议，subagent_type 填 host 内置通用类型如
   ...
   Task(subagent_type=<host 内置通用类型>,
        prompt="先完整 Read {{ subagents_root }}/project-fidelity-verifier.md，严格按其 Procedure 执行本轮任务。
                本轮 inputs：<task: re-verify whether my edits to evaluator.py / arch_codec.py / search_supernet.py drift from original project search semantics> + <my latest healed diff context> + Fixed:[<healed file list this round>] + Context: ns2_run_search self-heal。
                按 md 规定的格式 return。
...
搜索成功后、进 Step 3 前，运行 `select_architecture.py` 选架构。
select 结果落 `$ORCA_ARTIFACTS_DIR/.selected_arch.json` marker（供 ns2_report 读终态）。

🔴 **失败安全网**：select 崩 / rc≠0 / 无候选时，**不许**把 stderr 非 JSON 原样上抛（那会
output_schema 失败 → recoverable 3 次重试 → escalate workflow_failed，绕过 ns2_report）。
失败时 Step 3 emit falsy select 字段（`selected_arch=null, ..., select_reason="none"`）。
**select 失败不 node_failed**——ns2_report 从盘读 `.selected_arch.json` 正确归因 select_failed。
...
- **绝不手补假 JSON**：`status==failed` 就如实失败——节点 output_schema + 引擎双层判败。
  select 崩 → emit falsy 字段（`selected_arch=null`），禁 node_failed。
- **绝不带错下传**：缺上游 supernet ckpt / 禁碰-blocked → `status=failed`，让引擎终止，**不要**降级
  `executed` 让下游拿着空/坏 jsonl 跑。
...
`status==failed` → 引擎判 node 失败 → 路由 ns2_report。
