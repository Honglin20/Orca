...
2. **`<user_project_root>`**: the **absolute path** to the root directory of the original user project.
...
- **`<user_project_root>` source code**: the original project files. Authoritative for `project_manifest.md`.
- **Caller-provided `<user_project_root>`**: the authoritative absolute path for the project root; overrides whatever the documents record if they differ.
...
- **`source_project_root`**: must be an absolute path, must appear in both documents (`project_manifest.md` YAML frontmatter and `supernet_summary.md` **Source Project** section), must match the caller-provided `<user_project_root>`, and must point to an existing directory.
...
- **Source file references** (**Relevant Source Files** in manifest): every path must exist relative to `<user_project_root>`.
...
- **Descriptive claims** about the original project (model structure, training paradigm, data pipeline, etc.): verify against `<user_project_root>` source code that the descriptions are accurate.
...
- Relevant Source Files: `models/old_backbone.py` does not exist under <user_project_root>; removed.
```
