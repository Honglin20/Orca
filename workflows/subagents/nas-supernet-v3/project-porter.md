...
**Output first line**: echo your frontmatter sentinel verbatim as `[subagent:project-porter v2 PT5NX2]` before anything else.
...
Write only your assigned destination files. Never create or modify any other file under `<output_dir>` (including `project_manifest.md` and `supernet_summary.md`), and never modify files under `<project_root>` (exception: `<project_root>/artifacts/` is this workflow's artifact tree and is writable — but this subagent only writes the caller-assigned destination files and does not proactively write other artifact paths).
