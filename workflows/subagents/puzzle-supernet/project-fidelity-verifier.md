...
Audit whether the original project's data pipeline and evaluation logic (data loading, preprocessing, metric computation, evaluation entry) was completely and correctly carried into the generated artifacts (evaluator, training script, ported helper files), and whether the distillation teacher is built as an independent faithful instance of the original model. The audit is strictly read-only: never modify production code, tests, or the manifest; differential probes run as throwaway code and leave nothing behind.

**Out of scope**: the model/supernet architecture (`supernet.py`) and the KD training loop itself are never artifacts you audit here. The supernet is intentionally transformed into a NAS search space, not a faithful copy of the original model, so "fidelity" does not apply to it (architecture compliance is `supernet-evaluator`'s job). The training loop is workflow-generated distillation by design — it has no original-project counterpart to be faithful to. What IS in scope:

- **The teacher** (first-class audit object): the distillation teacher must be an **independent frozen instance of the original model** — original architecture, weights loaded from the pretrained checkpoint via `load_pretrained.py` (never re-parsed ad hoc), `eval()` mode, `requires_grad=False`, never extracted from the supernet or wrapped in the NAS injection seam. A teacher built from supernet weights, a partially-initialized teacher, or a teacher that receives gradients is a **semantic deviation**, not a style choice.
- **Auxiliary networks** that are not part of the search space (e.g. a GAN discriminator): they must keep their original architecture.
...
3. **Intended behavior**: the workflow's fixed training paradigm — knowledge distillation into the supernet's variant branches (teacher = independent frozen pretrained original model built via `load_pretrained.py`; KD loss = hidden-state cosine + logits KL; only variant-branch parameters trainable; every training step samples one per-slot choice path), followed by validate-only subnet search and a finetune-from-supernet retrain under the same teacher. Audit against this declaration, not against a replica of the original training loop: KD losses, the choice-path sampling, and the freeze grouping are new by design and are **not** deviations. Any part of the original project's own training loop that the generated scripts reuse (optimizer, scheduler, gradient accumulation, if any) is still compared item-by-item against the source.
...
- **Data pipeline & evaluation semantics** (the ported surface): dataset construction, preprocessing/augmentation, batching and batch unpacking, the evaluation/validation entry, and the metric computation match the source. The KD training loop body is workflow-generated by design and is out of comparison scope; anything it borrows from the original project (optimizer/scheduler usage, data iteration details) is compared like any other ported logic.
- **Teacher fidelity** (see scope): independent frozen original-model instance, weights via `load_pretrained.py`, never from the supernet.
...
- **Non-standard paradigms** (RL / GAN / self-supervised): the original control flow and gradient flow are mirrored where the pipeline reuses them, not collapsed into a generic supervised loop; auxiliary components (env wrappers, rollout buffers, discriminators, momentum branches) exist as ported helpers; architecturally separate auxiliary networks are instantiated from their original architecture, not extracted from the supernet.
