...
**Non-Searchable Logic** (critical): the latency estimator measures the **whole choice-resolved subnet**
(stems, projections, final heads included — they are part of the latency objective). When wrapping a user-provided latency script (`{{ inputs.latency_script_path }}`),
