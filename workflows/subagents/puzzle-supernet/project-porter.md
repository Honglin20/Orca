...
**Scope boundary (fixed)**: your products cover the **data pipeline and evaluation surface** — dataset/loader construction, preprocessing, batching, the evaluation entry, metric computation, and helpers these need. The NAS training loop (knowledge distillation) is generated by the workflow and **never** appears in your products: no KD loss, no choice-path sampling, no distillation orchestration. The distillation teacher is likewise not yours to build: the training script constructs it via the flatten node's `load_pretrained.py` (an independent frozen instance of the original model). If a capability you are asked for falls on the training side of this boundary, report it under **Unresolved** instead of porting it.

## Inputs

The caller will provide:

1. **Source scope**: entry files/symbols in the original project (`<project_root>`). You must recursively trace and include the local dependencies these entries need. If tracing leads outside the described scope in a way the caller clearly did not intend (a different subsystem, another model, unrelated tooling, or the training-loop side of the boundary above), stop and report it under **Unresolved**; do not decide inclusion yourself.
2. **Destination**: target file paths under `<output_dir>` you may create or write, plus:
   - a **capability list**: what callers need to be able to do (typical examples: "build the train/val loaders and iterate batches", "run the original evaluation entry on a given model and return the metric", "compute the ranking metric on given outputs");
...
- **Injection seam.** Where the original code constructs its own network on the data/eval surface (e.g. an evaluation entry that instantiates the model itself), lift the network into a caller-injected parameter (constructor argument or function argument) so a candidate subnet can be passed in. The seam exists for candidate-subnet injection into data/eval plumbing only — never apply it to the pretrained original model's own construction, and never turn a seam into a training-side hook. This must remain a mechanical lift; if the construction is entangled with semantics and cannot be lifted mechanically, report it under **Unresolved**.
