...
The **core** of this verifier is the adapter fidelity: `puzzle_adapters.py` must faithfully port the original project's data pipeline (`calib_iter` / `train_iter` / `extract_labels`), forward calling convention (`forward_model` + `FORWARD_CALLING_CONVENTION`), KD objective (`kd_loss` — the user's actual loss formula, not a hardcoded cosine / KL / MSE), task loss (`task_loss` — None only for genuinely unsupervised tasks), evaluation protocol (`evaluate` + `METRIC_DIRECTION` + `EVAL_NOISE_ATOL`), and ckpt load strategy (`load_pretrained` prefix stripping). `bld.py`'s normalized-MSE distillation (over the adapter-fed parent activations) and `gkd_retrain.py`'s end-to-end KD (over `adapters.kd_loss` + optional `adapters.task_loss`) must faithfully derive from the original project's training/evaluation semantics recorded in `project_manifest.md`. Drifts in loss formula, KD weight schedule, data sampling, or metric computation are semantic.
...
- **BLD objective fidelity**: `bld.py`'s normalized MSE `MSE(o_p, o_c) / MSE(o_p, 0)` distillation is intact — teacher is frozen parent, candidate block fed parent activations (via `adapters.forward_model` + `calib_iter`), denominator is the zero-output MSE (not a constant or dropped). Reweighting or replacing this objective is semantic.
- **GKD objective fidelity**: `gkd_retrain.py`'s KD loss is whatever `adapters.kd_loss(s_out, t_out, labels)` returns (the adapter faithful-ports the user's actual loss — contrastive / KL / cosine / MSE / task-specific). Swapping it for a hardcoded cosine / KL / MSE, or replacing the warmup schedule, is semantic. Likewise `adapters.task_loss` must faithful-port the user's hard-label supervision (None only for genuinely unsupervised tasks).
...
- **Evaluation-entry fidelity (mandatory, caller mapping or not)**: locate the original project's evaluation/validation function from the manifest's Evaluation entry (names like `eval_model`, `evaluate`, `test`, `validate`, or a standalone eval script) — never skip this. Trace it and compare against the adapter's `puzzle_adapters.py::evaluate` (which the generated `baseline_metrics.json` / `gate_report.py` path consumes) item by item: entry signature, reference/test data protocol, retrieval / sampling parameters (e.g. nearest-neighbor count, distance function — kept verbatim from the user's eval code), metric computation steps. The generated evaluation's returned/plotted metric must be the original function's metric under its real name.
