...
2. **Artifact paths / list**: files, directories, or script outputs produced or modified by this workflow run. For puzzle these include `project_manifest.md`, `block_map.json`, `baseline_metrics.json`, `<base>_flat.py`, `bld_summary.json`, `bld.py`, etc. These are the **only** files you may modify.
3. **Optional cross-references and context**: additional files for read-only inspection (e.g. `<base>_flat.py` for import/API consistency checks), invocation arguments, run logs, or extra verification instructions from the calling skill. This may include user-specified overrides such as the effective evaluation paradigm, custom metric names, or domain-specific constraints.

**Priority rule**: when the caller's context includes user-specified overrides or extra requirements, those take precedence over checklist items. If a checklist item contradicts the caller-provided context, treat the context as authoritative and mark the checklist item PASS (noting the override reason).
...
- **Discover companion checklist** (two-level discovery, stop at first hit):
  1. **Root level**: replace `workflows/` with `workflow-checklists/` in the workflow path and look for a file with the same basename (e.g. `workflow-checklists/puzzle.yaml.md`). If found, read it.
  2. **Agent-attached level**: if the root-level lookup misses, scan `workflows/agents/*/references/workflow-checklists/<basename>.md` (same basename as the workflow file, e.g. `puzzle.yaml.md`). Such checklists live alongside the agent that consumes them (puzzle's lives at `workflows/agents/pz_expand/references/workflow-checklists/puzzle.yaml.md`). If exactly one match is found, read it; if multiple match, prefer the one under the workflow's entry agent folder and note the disambiguation in the report.
  3. If neither level yields a file, proceed with the no-checklist fallback (step 2 below).

  Rationale: workflows are not required to ship a root-level `workflow-checklists/` directory; the authoritative checklist may be attached to an agent's `references/` tree. The two-level rule finds both layouts.
...
- **Companion checklist** (primary, when a companion checklist was found): each checklist item becomes a todolist entry titled `[N] <title> (SEVERITY)`, e.g. `[1] block_map.json exists and has ≥1 slot (CRITICAL)`. Do **not** scan the workflow for extra requirements; gaps belong in the checklist.
...
**Item ID** is the leading bracket token of the todolist title (`[1]`, `[CROSS-REF-1]`, `[BUILT-IN-1]`, …), stable for this audit instance; do not renumber. The full todolist title (ID plus the rest, e.g. `[1] block_map.json exists and has ≥1 slot (CRITICAL)`) is reused verbatim as the Fixed/Unresolved block header (see **Output**).

## Resumed Re-Check Mode

When resumed with a list of Item IDs (e.g. `Fixed: [1], [CROSS-REF-1]`): re-check only those IDs (match on the leading `[…]` token); re-run static checks on files modified since the previous audit; return the standard report for the re-checked items only, using the same full title. Do not rebuild the full todolist.
...
4. **Fixed** (only if any): under this section heading, one block per item. The block opens with the **same full title** as the todolist entry, then a flat markdown list (not nested) for finding, file, change summary, and why the fix was allowed. Confirm static checks passed. No diffs.
