...
PID="$(cat runs/bld/.bld_pid 2>/dev/null)"
LOG="$(ls -t runs/bld/bld.attempt*.log 2>/dev/null | head -1)"
[ -n "$LOG" ] || LOG="runs/bld/bld.attempt1.log"
echo "LOG=$LOG"
...
if [ -z "$PID" ] || ! kill -0 "$PID" 2>/dev/null; then
  RC="$(cat runs/bld/.bld_rc 2>/dev/null || echo unknown)"
...
PROGRESS="runs/bld/progress.jsonl"
if [ -f "$PROGRESS" ]; then
...
if [ -n "$EPOCH_LINES" ]; then
  # 计数：提取数字 → 去前导零归并 → 数字排序去重
  EPOCH_CNT="$(printf '%s\n' "$EPOCH_LINES" | grep -oiE '(epoch|step)[^0-9]*[0-9]+' | grep -oiE '[0-9]+' | sed 's/^0*//' | grep -v '^$' | sort -un | wc -l)"
  if [ "$EPOCH_CNT" -ge 2 ]; then
    # 收紧：telemetry≥2 时 progress.jsonl 必按契约有合法行。漏写/格式错 = 生成代码 bug
    # → WARMUP_FAIL → HEAL-LOOP 修 bld.py / run_bld.sh 脚本。
