...
#   每 60s 与 BLD 抢 CPU/显存（CRON 模型 1~2h 才查一次，本模型高频，须省）。仅当进程退出（rc 出现或 pid 死）
...
#   *COMPLETE* ckpt=<path>   → agent: Step4 executed
#   GATE_SKIP                 → agent: Step4 skipped
...
# ── 路径（pz_build_library）──
ATT="$(cat runs/bld/.bld_attempt 2>/dev/null || echo 1)"
LOG="runs/bld/bld.attempt${ATT}.log"
PIDF="runs/bld/.bld_pid"; RCF="runs/bld/.bld_rc"
LAST_SIZE=0; STALL=0
...
while [ "$SECONDS" -lt "$DEADLINE" ]; do
  ...
  if tail -n 50 "$LOG" 2>/dev/null | grep -Eqi '(^|[[:space:]:=,(])(nan|inf|overflow|out of memory|cuda error|runtime[[:space:]]*error|traceback|exception|error[[:space:]]*:)([[:space:],).:]|$)'; then
    echo "BLD_STUCK nan-or-error-in-log"; exit 0
  fi
  if [ "$CUR_SIZE" -le "$LAST_SIZE" ]; then
    STALL=$((STALL + 1))
    if [ "$STALL" -ge "$STALL_POLLS" ]; then echo "BLD_STUCK log-stalled polls=$STALL"; exit 0; fi
  else
    STALL=0
  fi
  LAST_SIZE="$CUR_SIZE"
