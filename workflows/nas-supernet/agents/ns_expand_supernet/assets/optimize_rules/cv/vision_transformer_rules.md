...
3. Do not add any replacement position encoding at the model level. Position information is handled internally by each block's attention mechanism (e.g., relative position bias for window attention, neighborhood structure for NAT, conditional position encoding for other blocks).
