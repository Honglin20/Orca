...
1. AGENTS.md 不存在 → failed（上游 scaffold 缺失，无从生成 retrain 脚本——前置错误）
2. 训练**真正完成**（.retrain_rc==0 且训练进程已退出 且 ckpt 存在 + torch.load 可读）→ executed
3. 否则 → failed（尝试预算耗尽 / self-heal 耗尽 / 前置失败，如 3b fidelity 禁碰清单拒改；
...
agents_md = os.path.join(ad, "AGENTS.md")
