...
* The user-model-derived block should preserve the original block's high-level computation structure:
  * convolution / normalization / activation ordering
  * internal expansion or bottleneck pattern
  * Adaptations for the Block I/O contract below (adding `self.downsample`, adopting identity residual) are expected and do not count as structural deviations.
...
* When building the user-model-derived elastic block, use the loaded pre-built CNN block source files as few-shot structural references for this Block I/O pattern, not as sources for parameter conventions.
...
* The exported subnet must preserve the same fixed stem, hard representation-changing modules, task head, and the same searchable stage structure as the active supernet path. Fixed modules that carry parameters, and any buffer or parameter registered directly on the supernet or a fixed module (`register_buffer`, `register_parameter`, or a bare `nn.Parameter` attribute), should be deep-copied (`copy.deepcopy`) into the exported subnet.
