...
* The exported subnet must preserve the same fixed components (patch embedding, positional embedding, `cls_token`, final norm, task head) and the same searchable layer structure as the active supernet path. Fixed modules that carry parameters, along with standalone buffers/parameters such as the positional embedding and `cls_token` (`register_buffer`, `register_parameter`, or a bare `nn.Parameter` attribute), should be deep-copied (`copy.deepcopy`) into the exported subnet.
