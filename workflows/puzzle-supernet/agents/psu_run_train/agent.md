---
description: Run the puzzle-supernet supernet training to true completion with self-healing monitoring and resumable long-running turns.
...
The upstream `psu_train_script` has already produced the training script in `$ORCA_ARTIFACTS_DIR` (possibly including `run_train_supernet.sh`).
...
- You supervise training end-to-end via **bounded polling** (`monitor_until_done.sh`, ~9min/block): continuously issue
  ...
  on the next turn through the Step 1 status.sh source-of-truth.
...
- **Environment prerequisites** (for "scripts run-only, do not modify"): a Linux training machine with bash + python3 +
  coreutils (`setsid`/`nohup`/`kill`). If these are missing, fail loud in the assessment — do not adapt the run-only scripts.
...
- `$ORCA_ARTIFACTS_DIR` (injected by orca spawn) = this run's artifacts directory, where the upstream
  psu_train_script lands scripts, shared across nodes.
- `$ORCA_AGENT_RESOURCES` (injected by orca spawn) = this agent's resource directory, i.e. the directory containing this
...
- After each `edit` to a whitelisted file:
  `bash -c 'printf "%s\n" "<edited_file_relpath>" >> "$ORCA_ARTIFACTS_DIR/.psu_run_train_healed.txt"'`
- After Step 3f fidelity-verifier runs (whatever the pass/fail conclusion):
  `printf "true" > "$ORCA_ARTIFACTS_DIR/.psu_run_train_fidelity.flag"`
- Soft judgment / pre-completion assessment (Step 3d / 3b):
  `printf "%s" "<one-line assessment>" > "$ORCA_ARTIFACTS_DIR/.psu_run_train_assessment.txt"`
...
1. **Script missing is a fail-loud, not a skip**: `status.sh` outputs `TRAIN_SCRIPT_MISSING` (`run_train_supernet.sh`
   absent = upstream `psu_train_script` failed to produce it) → go straight to Step 4 and output `{"status":"failed"}`
   with that reason, **do not** fabricate execution.
...
5. **Forbidden list (hard iron rule; violation = architectural breakage, the only failed trigger)**: the following files are **read-only, no edit/write** —
   ...
   **source files** under `{{ inputs.project_root }}` (**exception**: `{{ inputs.project_root }}/artifacts/`
   ...
   last_error in `.psu_run_train_assessment.txt`, go to Step 4 and output `{"status":"failed"}`.
...
|---|---|---|
| Step 1 | Run `status.sh` (combined gate + complete + alive) | `TRAIN_SCRIPT_MISSING` → Step 4 failed; `TRAIN_COMPLETE` → Step 4 executed; `TRAIN_ALIVE` → Step 2; `TRAIN_INCOMPLETE` → Step 3 |
...
- `TRAIN_SCRIPT_MISSING` → go straight to Step 4 and output `{"status":"failed"}` (script missing = upstream generation failure). **Do not** fabricate execution.
- `TRAIN_COMPLETE ckpt=<path>` → go straight to Step 4 (whose completion-curve guarantee runs the watcher once before
  emit — this run may be reusing artifacts trained by a previous run) and output `{"status":"executed","artifacts":["<path>"],...}`
...
- `WARMUP_FAIL reason=process-exit rc=0` → **training already ran to completion normally within the warmup window** (not a failure):
  re-run `status.sh` — if `TRAIN_COMPLETE`, go straight to Step 4 and output executed (the Step 4 completion-curve guarantee
  covers this fast path — the watcher drains and pushes/persists on its already-done exit); otherwise (invalid ckpt etc.)
  enter the HEAL-LOOP.
  ...
  **Cap of 5** (~20 min); if still no progress marker (epoch/step) beyond the cap, split on whether the log is growing:
  - **log growing** (`LOG_MTIME`/`LOG_SIZE` changing between two calls, or tail continuously has content) →
    **fallback for uncontracted log format**: `read` the log and judge health manually (loss decreasing / training progress output → healthy;
    ...
    for psu_train_script contract investigation).
...
> WARMUP_OK semantics: the first 1~2 progress markers (epoch/step, per the psu_train_script generated contract) appearing =
> proof that training **can run through** (data pipeline, model forward/backward, and a writable ckpt directory all passed).
...
3. Judge which layer the root-cause fix belongs to:
   ...
   - **Root cause requires editing the forbidden list** (`supernet.py` / `project_manifest.md` / `supernet_summary.md` / source files)
     → **the only failed path**: do not touch, record last_error in `.psu_run_train_assessment.txt`,
...
1. Invoke the host's built-in generic subagent (point-to-file protocol; set subagent_type to the host's built-in generic type such as
   ...
   Task(subagent_type=<host built-in generic type>,
        prompt="First Read {{ subagents_root }}/project-fidelity-verifier.md in full, then strictly execute this round's task per its Procedure.
                This round's inputs: <task: re-verify whether my edits to train_supernet.py / evaluator.py drift from original project training semantics> + <my latest healed diff context> + Fixed:[<healed file list this round>] + Context: psu_run_train self-heal.
   ...
   `" | fidelity-verifier subagent body not deployed; cannot retrigger"` to the end of `.psu_run_train_assessment.txt`, and skip this step.
2. Merge the verifier conclusion (pass / fail + reason) into `.psu_run_train_assessment.txt`;
   `printf "true" > .psu_run_train_fidelity.flag` (**regardless of verifier pass/fail** — as soon as it is re-triggered, mark true;
...
- `TRAIN_SCRIPT_MISSING` → go to Step 4 and output failed.
...
> from `train.attempt*.log` + `.psu_run_train_healed.txt` + `train_status.md` — read them to judge
...
Only three situations enter this step: Step 1 hits `TRAIN_SCRIPT_MISSING` / `TRAIN_COMPLETE` / forbidden-list-blocked failed.

**Completion-curve guarantee (contract, every completion path)**: before `emit_result.py`, this node must ensure the training
curves of this attempt have been pushed or persisted — run the watcher once (idempotent; returns in seconds):

```bash
python3 "$ORCA_AGENT_RESOURCES/scripts/progress_watcher.py" --progress "runs/train/progress.jsonl" --done-marker "runs/train/.train_rc" --label "puzzle-supernet/train" || true
```

This applies to **every** path that reaches Step 4 with training complete — the Step 1 direct `TRAIN_COMPLETE`
(this run reusing artifacts a previous run trained; the watcher never ran this run), the 3b warmup-window fast path,
and the C-loop `*COMPLETE*` exit. When the attempt is already done the watcher exits immediately after one final
push/persist; when live push is unavailable it writes static charts under `charts/`. Failure does not block emit
(`|| true`), but record it truthfully in `.psu_run_train_assessment.txt` — a completed training with zero pushed
or persisted curves is a contract breach, not a cosmetic miss.
...
status derivation (inside emit_result.py): `failed` (script missing — upstream generation failure) / `executed` (rc=0 + process exited +
...
emit_result.py yields failed automatically). The deterministic part is judged from the real filesystem;
...
- **Never hand-write fake JSON**: if `status==failed`, fail truthfully — node `output_schema` validation + the engine's fail routing
  to `psu_report` (short-circuit, correctly attributing train_failed) make fabrication pointless; tape audit + marker files are traceable.
- **Never propagate an error downstream**: forbidden-list-blocked → `status=failed`. The yaml routing contract: failed short-circuits to `psu_report` (no cascade
  to search) — **do not** downgrade to `executed` and let downstream run with a bad ckpt.
- **Incomplete ≠ finished**: while training is incomplete, output a status message (not JSON), **do not** submit "training" as executed/failed
...
- **Completion-curve guarantee**: every path entering Step 4 with training complete must have run the watcher once
  ...
  A completed training with zero pushed or persisted curves is a contract breach; record any watcher failure
  truthfully in `.psu_run_train_assessment.txt`.
...
`status==failed` → short-circuit to `psu_report` (correctly attributing train_failed, no cascade to search). **When training is incomplete, the whole reply = a status message (containing
