...
# pid 文件 / status.sh 报的 pid 可能是**另一 run** 的训练 wrapper —— 名字级 cmdline 匹配
...
#   - 别的 run 的训练 → **不杀**，stdout `FOREIGN_RUN_ALIVE` + exit 1（调用方按 FOREIGN 语义处理）
#
# 退出码契约：0 = 已处理（已杀 / 无需杀）；1 = FOREIGN（另一 run 训练中，未杀）。
...
# cmdline 校验防 PID 复用误杀：必须含本 workflow 的训练脚本名（wrapper 是
# `setsid nohup bash -c '…bash run_train_supernet.sh…'`，脚本文本留在 cmdline 里）。
...
if [ -n "${ORCA_RUN_ID:-}" ] && [ -n "$PREV_RUN" ] \
  ...
  echo "FOREIGN_RUN_ALIVE pid=$PID（另一 run 的训练在跑；跨 run 杀进程已禁用）"
  exit 1
fi

# 本 run 残留（或 ORCA_RUN_ID 缺失的 legacy 场景）→ 整组杀。launch.sh 用 setsid 起进程组，
# `kill -- -PID` 整组杀——含训练 python，防"只杀 wrapper、孤儿训练进程残留 → 下轮重复 detach"。
