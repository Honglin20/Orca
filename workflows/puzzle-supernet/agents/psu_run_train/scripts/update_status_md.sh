#!/bin/bash
# update_status_md.sh —— 写 $ORCA_ARTIFACTS_DIR/train_status.md（跨唤醒真相源；与上游节点
...
# MD 的 epoch 反映 log 最新进度）；total/per_epoch 参考 .train_eta.json 作 ETA。
...
ATTEMPT="$(cat runs/train/.train_attempt 2>/dev/null || echo 1)"
...
eta_path = os.path.join(ad, "runs", "train", ".train_eta.json")
...
cat > train_status.md <<EOF
# train_status.md — psu_run_train 训练状态（本节点维护，跨唤醒真相源）

- status: $STATUS
- attempt: $ATTEMPT
- pid: $PID
- log: runs/train/train.attempt$ATTEMPT.log
- $EPOCH_SUMMARY
