...
# 进度行契约见 psu_train_script references train_supernet_script_generation.md Logging 节。
set +e
...
if [ -f "runs/train/progress.jsonl" ]; then
  grep -E ':[[:space:]]*-?(NaN|Infinity)' runs/train/progress.jsonl 2>/dev/null | tail -2
else
