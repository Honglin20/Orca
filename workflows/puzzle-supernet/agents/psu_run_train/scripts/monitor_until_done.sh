...
#   TRAIN_SCRIPT_MISSING      → agent: Step4 failed（仅 train；脚本缺失=上游生成失败，fail loud）
...
# ── 路径（psu_run_train 镜像）──
ATT="$(cat runs/train/.train_attempt 2>/dev/null || echo 1)"
LOG="runs/train/train.attempt${ATT}.log"
PIDF="runs/train/.train_pid"; RCF="runs/train/.train_rc"
LAST_SIZE=0; STALL=0
  ...
  if [ "$RC_EXISTS" = "1" ] || [ "$ALIVE" = "0" ]; then
    ...
    case "$out" in
      *COMPLETE*)   echo "$out"; exit 0;;
      TRAIN_SCRIPT_MISSING*) echo "$out"; exit 0;;
