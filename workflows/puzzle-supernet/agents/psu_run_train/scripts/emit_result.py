#!/usr/bin/env python3
"""emit_result.py —— psu_run_train 最终 JSON（**唯一产出节点 JSON 的时刻**）。

status 推导优先级（互斥，先命中先定）：
1. run_train_supernet.sh 不存在 → failed（上游 psu_train_script 未产出，fail loud 非 skip）
...
ckpt_marker = os.path.join(ad, ".psu_run_train_ckpt_resolved.txt")
...
if not os.path.exists(script_path):
    ...
    if log_tail:
        prev = read_text(os.path.join(ad, ".psu_run_train_assessment.txt"), "")
        with open(os.path.join(ad, ".psu_run_train_assessment.txt"), "w", encoding="utf-8") as fh:
            fh.write((prev + "\n" if prev else "") + "last_error:\n" + log_tail)

healed_files = read_lines(os.path.join(ad, ".psu_run_train_healed.txt"))
fidelity_retriggered = read_text(os.path.join(ad, ".psu_run_train_fidelity.flag"), "false") == "true"
assessment = read_text(os.path.join(ad, ".psu_run_train_assessment.txt"), "no assessment recorded")
