#!/usr/bin/env python3
"""emit_result.py — psu_run_search final JSON (the only producer of the node output).
...
if script_exists and recs >= 1:
    ...
    if log_tail:
        prev = read_text(os.path.join(ad, ".psu_run_search_assessment.txt"), "")
        with open(os.path.join(ad, ".psu_run_search_assessment.txt"), "w", encoding="utf-8") as fh:
            fh.write((prev + "\n" if prev else "") + "last_error:\n" + log_tail)

healed_files = read_lines(os.path.join(ad, ".psu_run_search_healed.txt"))
fidelity_retriggered = read_text(os.path.join(ad, ".psu_run_search_fidelity.flag"), "false") == "true"
assessment = read_text(os.path.join(ad, ".psu_run_search_assessment.txt"),
