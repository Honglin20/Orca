#!/usr/bin/env bash
# precheck.sh — psu_run_search Step 1 pre-checks.
...
rm -f .psu_run_search_healed.txt .psu_run_search_fidelity.flag .psu_run_search_assessment.txt

if [ ! -f run_search_supernet.sh ]; then
  printf "FATAL: run_search_supernet.sh absent — psu_search_pipeline did not produce it." \
    > .psu_run_search_assessment.txt
