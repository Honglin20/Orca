#!/usr/bin/env python3
"""full_supernet_latency.py -- Measure the all-original path latency (baseline anchor).

The search measures per-candidate latency (each is an active subnet extracted from the
supernet). The latency of the all-original path (every slot on the frozen original branch
≈ the pretrained baseline model) was historically proxied by ``max(candidate latencies)``
(``compare_table.py`` fallback). This script produces a REAL measurement using the
SAME ``LatencyEstimator`` the search used, on ``SearchSpace().all_original()``
(the default anchor configuration), and writes ``.full_supernet_latency.json`` for
...
def main() -> int:
    ...
    # pass --latency-script-path, so this script stays byte-identical across puzzle-supernet
...
def _measure(ad: Path) -> float:
    ...
    # The anchor configuration is the all-original path: every slot chooses the
    # frozen original branch (spec: SearchSpace.all_original()). SuperNet() above is
    # instantiated as a constructiveness/symbol check and sets the same active config.
    anchor_arch = search_space.all_original()
    ...
    latency = estimator.get_latency(anchor_arch)
