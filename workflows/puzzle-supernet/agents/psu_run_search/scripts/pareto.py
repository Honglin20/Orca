#!/usr/bin/env python3
"""pareto.py -- Pareto front scatter (latency vs metric) for puzzle-supernet.

Reads search_results.jsonl + selected-arch coords (from psu_select output via CLI args).
...
def main() -> int:
    ...
    if not records:
        push_chart(
            artifacts_dir_path=ad, script_name="pareto", label="puzzle-supernet/search",
