"""_common.py -- shared utilities for puzzle-supernet chart scripts.
...
- Static-file fallback: when the Orca chart socket is unavailable (headless / post-run
  rendering), push_chart writes a self-contained static file under
  ``<artifacts>/charts/<script_name>.{html,png}`` and records status="rendered_static"
  with the path. Render ladder: plotly HTML → matplotlib PNG → **zero-dependency
  pure-HTML/SVG floor (stdlib only — plotly and matplotlib both missing still renders;
  a real headless-box incident had charts silently vanish)**. Lets runs whose charts
  were skipped after completion be re-rendered from their own artifacts. Static path
...
def _render_static(
    ...
    try:
        ...
        return None, str(exc)
    except Exception as exc:  # noqa: BLE001 -- try the zero-dependency floor next
        mpl_reason = f"matplotlib failed: {exc}"
        sys.stderr.write(f"[{script_name}] matplotlib render failed for '{title}': {exc}\n")
    # Zero-dependency floor: pure-HTML/SVG render (stdlib only). plotly 与 matplotlib
    # 均不可用时也必须落盘（此前两轴皆缺 = 静默 skip，图表凭空消失）。
    try:
        return _render_pure_html(
            artifacts_dir_path, script_name, title, chart_type, data, render_kwargs,
        )
    except _UnsupportedChartType as exc:
        return None, str(exc)
    except Exception as exc:  # noqa: BLE001 -- fail-soft: caller records "skipped"
        floor_reason = f"pure-html failed: {exc}"
        sys.stderr.write(f"[{script_name}] pure-html render failed for '{title}': {exc}\n")
        combined = "; ".join(r for r in (plotly_reason, mpl_reason, floor_reason) if r)
...
def _render_pure_html(
    ...
    """Zero-dependency static floor: self-contained plain-HTML/SVG chart (stdlib only).

    Guaranteed last-resort renderer after plotly and matplotlib both fail/are
    missing (headless box without either installed — a real E2E incident: charts
    silently vanished). Supports the same subset as the plotly static path:
    {line, bar, pareto/scatter, table}. Raises ``_UnsupportedChartType`` for other
    types (kept loud so the caller records a precise skip reason).
    """
    import html as _html  # noqa: PLC0415 -- stdlib, escaped output
    ...
    if chart_type == "table":
        columns = kw.get("columns") or (list(data[0].keys()) if data else [])
        note = _html.escape(caption) if caption else f"{len(data)} rows"
        parts.append(
            f"<p style='color:#555;font-size:13px'>{note} — static fallback table "
            "(live push unavailable; plotly/matplotlib missing)</p>"
        )
        parts.append(
        ...
        for d in data:
            parts.append(
                "<tr>"
                + "".join(f"<td>{_html.escape(_fmt_cell(d.get(c, '')))}</td>" for c in columns)
                + "</tr>"
            )
        parts.append("</tbody></table>")
    elif chart_type == "bar":
        # Categorical x: bars at index positions with the raw x value as the tick
        # label (bar x is often a phase/category name, not a number).
        w, h, ml, mt = 960, 480, 90, 60
        pw, ph = w - ml - 50, h - mt - 90
        cats: list[str] = []
        vals: list[float] = []
        for d in data:
            yv = _num(d.get(y))
            if yv is None:
                continue
            cats.append(str(d.get(x, "")))
            vals.append(yv)
        if not vals:
            raise ValueError(f"no numeric {y!r} values in {len(data)} records")
        ymin, ymax = min(vals), max(vals)

        def _by(v: float) -> float:
            return mt + (1 - (v - ymin) / ((ymax - ymin) or 1.0)) * ph
        ...
        svg = [
            ...
            f"<text x='{ml}' y='{mt - 14}' font-size='12' fill='#333'>"
            f"{_html.escape(f'{y_label} max={ymax:g}')}</text>",
            f"<text x='12' y='{int(mt + ph / 2)}' font-size='12' fill='#333'>"
            f"{_html.escape(f'{y_label} min={ymin:g}')}</text>",
        ]
        slot = pw / len(vals)
        bw = slot * 0.6
        for i, (cat, v) in enumerate(zip(cats, vals)):
            bx = ml + slot * i + (slot - bw) / 2
            svg.append(
                f"<rect x='{bx:.1f}' y='{_by(v):.1f}' width='{bw:.1f}' "
                f"height='{mt + ph - _by(v):.1f}' fill='#2563eb' opacity='0.85'/>"
            )
            svg.append(
                f"<text x='{ml + slot * i + slot / 2:.1f}' y='{mt + ph + 20}' font-size='11' "
                f"fill='#333' text-anchor='middle'>{_html.escape(cat[:24])}</text>"
            )
        svg.append("</svg>")
        parts.extend(svg)
        if caption:
            parts.append(f"<p style='color:#555;font-size:13px'>{_html.escape(caption)}</p>")
        parts.append(
            f"<p style='color:#555;font-size:13px'>{len(vals)} bar(s). Static fallback chart "
            "(live push unavailable; plotly/matplotlib missing).</p>"
        )
    elif chart_type in ("line", "pareto", "scatter"):
        w, h, ml, mt = 960, 480, 90, 60
        pw, ph = w - ml - 50, h - mt - 70
        pts: list[tuple[float, float]] = []
        for d in data:
            xv, yv = _num(d.get(x)), _num(d.get(y))
            if xv is not None and yv is not None:
                pts.append((xv, yv))
        front_pts: set[tuple[float, float]] = set()
        if chart_type in ("pareto", "scatter"):
            for i in _compute_pareto_front(
                data, x, y,
                kw.get("pareto_x_direction", "min"),
                kw.get("pareto_y_direction", "min"),
            ):
                xv, yv = _num(data[i].get(x)), _num(data[i].get(y))
                if xv is not None and yv is not None:
                    front_pts.add((xv, yv))
        sel = _parse_selected_from_caption(caption) if chart_type in ("pareto", "scatter") else None
        if sel is not None:
            pts.append(sel)
        if not pts:
            raise ValueError(f"no numeric ({x}, {y}) points in {len(data)} records")
        ...
        def _sx(v: float) -> float:
            return ml + (v - xmin) / ((xmax - xmin) or 1.0) * pw
        ...
        svg = [
            ...
            f"<text x='{ml}' y='{mt - 14}' font-size='12' fill='#333'>"
            f"{_html.escape(f'{y_label} max={ymax:g}')}</text>",
            f"<text x='{ml}' y='{mt + ph + 18}' font-size='12' fill='#333'>"
            f"{_html.escape(f'{x_label} min={xmin:g}')}</text>",
            f"<text x='{ml + pw}' y='{mt + ph + 18}' font-size='12' fill='#333' text-anchor='end'>"
            f"{_html.escape(f'max={xmax:g}')}</text>",
            f"<text x='12' y='{int(mt + ph / 2)}' font-size='12' fill='#333'>"
            f"{_html.escape(f'{y_label} min={ymin:g}')}</text>",
        ]
        if chart_type == "line" and len(pts) > 1:
            svg.append(
                "<polyline fill='none' stroke='#2563eb' stroke-width='2' points='"
                + " ".join(f"{_sx(a):.1f},{_sy(b):.1f}" for a, b in pts) + "'/>"
            )
        for a, b in pts:
            if sel is not None and (a, b) == sel:
                svg.append(
                    f"<circle cx='{_sx(a):.1f}' cy='{_sy(b):.1f}' r='8' fill='#d97706' "
                    "stroke='#111' stroke-width='1.5'/>"
                )
            elif (a, b) in front_pts:
                svg.append(f"<circle cx='{_sx(a):.1f}' cy='{_sy(b):.1f}' r='6' fill='#dc2626'/>")
            else:
                svg.append(f"<circle cx='{_sx(a):.1f}' cy='{_sy(b):.1f}' r='4' fill='#2563eb'/>")
        svg.append("</svg>")
        parts.extend(svg)
        if caption:
            parts.append(f"<p style='color:#555;font-size:13px'>{_html.escape(caption)}</p>")
        parts.append(
            f"<p style='color:#555;font-size:13px'>{len(pts)} point(s). Static fallback chart "
            "(live push unavailable; plotly/matplotlib missing).</p>"
        )
    else:
        raise _UnsupportedChartType(
            f"static pure-html render does not support chart_type={chart_type!r} "
            f"(supported: line, bar, pareto/scatter, table)"
        )
...
def push_chart(
    ...
    **render_kwargs: Any,
    ...
    Args:
        label: Dedup/grouping key (e.g. ``"puzzle-supernet/search"``). Same ``label`` +
