---
description: Terminal reporter for the puzzle-supernet pipeline that reads on-disk status files under $ORCA_ARTIFACTS_DIR, determines the terminal state, and emits the single final structured report JSON.
tools: [bash, read]
---
# psu_report
...
1. **Immediately run the bash in Step 1** (`cd "$ORCA_ARTIFACTS_DIR"` + python script). **Do not** first
   `ls` the repo / `cat` yaml / `read` this agent.md / `git status` / explore unrelated directories — those are
...
You are the **only terminal reporter** of the puzzle-supernet pipeline. All paths (success + 8 failure modes)
...
## Data-source hard rule

**Do not** reference other nodes' output fields (e.g. psu_retrain / psu_run_search outputs) — those nodes
may not have run on failure paths, so their outputs do not exist. Use only inputs fields (always defined)
and on-disk files under `$ORCA_ARTIFACTS_DIR` (via bash/read) to determine the terminal state.

## Terminal-state reference (informational only — the Step 1 python decides)

The first-match-wins terminal-state determination is implemented deterministically in the Step 1 python (`emit_report.py`), which reads the on-disk status files itself. **You never judge the terminal state** (hard rule 3). The table below is reading material to help you understand the report fields — if it ever seems to disagree with the Step 1 python's output, the python wins:
...
| `unsupported` | Double signal: `.psu_expand_unsupported.flag` content = `'true'` **OR** `supernet_summary.md` contains the `No supported match` substring | failed | expand |
| `original_equivalence` | `.equivalence.json` exists with `passed=false` (the expand equivalence gate writes it on BOTH pass and fail) — all-original path output ≠ pretrained original model (weight inheritance / freeze grouping / choice container broken) | failed | expand |
| `expand_crashed` | `supernet.py` exists **AND** `.equivalence.json` missing **AND** no downstream artifacts (`search_results.jsonl` / `.train_rc` / `.retrain_rc` all absent) | failed | expand |
| `training_prerequisites_missing` | `supernet.py` exists (expand succeeded) **AND** `train_supernet.py` + `run_train_supernet.sh` both missing (viable=false produced no scripts) **AND** `.train_rc` missing **AND** `search_results.jsonl` missing | failed | train_script |
...
**Your entire final reply = the one line of JSON printed by the Step 1 python.** output_schema enforces 15 fields.
...
- `selected_arch`: the selected subnet architecture — a per-slot branch choice vector, one branch name per transformer layer slot (read from `.selected_arch.json`; null if absent).
- `selected_acc/selected_latency/latency_unit/pareto_size`: read from `.selected_arch.json` (latency_unit defaults to ms).
- `subnet_structure`: relative path of `subnet_structure.md` (produced by psu_retrain pushing subnet_profile.py; empty string if missing/materialization failed).
...
- `artifacts`: list of key artifact paths (includes `runs/train/supernet_best.pth` — the trained supernet ckpt — when present).
