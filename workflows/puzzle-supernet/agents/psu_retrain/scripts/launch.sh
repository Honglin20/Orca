...
# → `.retrain_pid` 里可能是**别的 run** 的训练 wrapper。统一走带 run 归属门的
...
# `.psu_retrain_exec_healed.txt` 每 attempt 清——执行期补丁痕迹重新记）。
rm -f .psu_retrain_exec_healed.txt .psu_retrain_assessment.txt .psu_retrain_ckpt_resolved.txt runs/retrain/progress.jsonl runs/retrain/.retrain_rc
...
# --title 省略：watcher 自推导 "<label> (attempt N)"（N 读本 attempt 写入的 .retrain_attempt），
# 与 agent 完成路径补跑同推导 → 同 label+attempt 同 title（dedup 替换语义）。
setsid nohup bash -c 'python3 "$ORCA_AGENT_RESOURCES/scripts/progress_watcher.py" --progress "runs/retrain/progress.jsonl" --done-marker "runs/retrain/.retrain_rc" --label "puzzle-supernet/retrain" >/dev/null 2>&1 & bash run_retrain.sh > "runs/retrain/retrain.attempt'"$N"'.log" 2>&1; echo $? > runs/retrain/.retrain_rc' >/dev/null 2>&1 &
