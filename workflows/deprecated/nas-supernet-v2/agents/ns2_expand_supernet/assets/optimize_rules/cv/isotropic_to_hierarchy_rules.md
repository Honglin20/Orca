...
- `LayerNorm → Window Attention (with RPB) → Residual`
...
See `../supernet_readiness/hierarchical_transformer.md` for structural rules (Spatial Layout Conversion, Convolutional Stage Downsampling) and `vision_transformer_rules.md` for model-level adaptations (Remove Position Embeddings, Global Average Pooling Head).
