...
**Fix**: Only when latency is already measured into a local variable (e.g. `latency_ms`) and the candidate loop print merely omits it: append that value to the existing print/f-string (e.g., `, latency: {latency_ms:.2f}ms`). If measurement logic itself is missing, report under Unresolved — do not invent a measurement path.
