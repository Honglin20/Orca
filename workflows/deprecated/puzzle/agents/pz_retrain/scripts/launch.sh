#!/bin/bash
# launch.sh —— detach（一次短调用，秒级返回，禁 wait/sleep）。
# attempt 记在 .retrain_attempt（仅 log 命名计数 + 审计，**无上限**——不阻断 detach）。
# 依赖：ORCA_ARTIFACTS_DIR（orca spawn / orca_env.sh 注入）+ setsid（Linux 训练机既有假设）。
set -e
cd "$ORCA_ARTIFACTS_DIR" || { echo "FATAL: ORCA_ARTIFACTS_DIR unreachable" >&2; exit 1; }
mkdir -p runs/retrain

# ── 残留进程归属判定（防跨 run 误杀）─────────────────────────────────────────
# 同项目并发 run 共享 artifacts 目录（engine project-scoped `<project_root>/artifacts/<wf>`）
# → `.retrain_pid` 里可能是**别的 run** 的 GKD wrapper。统一走带 run 归属门的
# kill_train_group.sh（与 agent Step 2 假死 / self-heal 同源）：本 run 残留才整组杀；
# 别的 run 的活 GKD → FOREIGN_RUN_ALIVE + exit 1 abort（不 kill；abort 在 attempt 计数
# **之前**）。cmdline/PID 复用防误杀在脚本内。
PREV_PID="$(cat runs/retrain/.retrain_pid 2>/dev/null || echo '')"
if [ -n "$PREV_PID" ] \
   && ! bash "$ORCA_AGENT_RESOURCES/scripts/kill_train_group.sh" "$PREV_PID"; then
  exit 1
fi

# N = 上次 attempt + 1（首启无记录 → N=1）。无上限——仅 log 命名 + 审计计数。
PREV="$(cat runs/retrain/.retrain_attempt 2>/dev/null || echo 0)"
N=$((PREV + 1))
echo "$N" > runs/retrain/.retrain_attempt

# 清本 run 审计痕迹（续训**不**删 ckpt，只清 marker；progress.jsonl 每 attempt 清零；
# .retrain_rc 也要清——resume 时 stale rc 残留会让 monitor 每 60s 误判进程退出、旁路 cheap 活性；
# `.pz_retrain_generated.txt` 保留——生成产物跨 attempt 复用，不重新生成；
# `.pz_retrain_fidelity.flag` **不清**——3b 首启 fidelity 在 launch 前已写，清掉会让成功路径
# fidelity_retriggered 失真；flag 是覆盖写（`printf "true" >`），无 stale，语义 = "对当前脚本已跑过 fidelity"）。
rm -f .pz_retrain_healed.txt .pz_retrain_assessment.txt .pz_retrain_ckpt_resolved.txt runs/retrain/progress.jsonl runs/retrain/.retrain_rc

# detach：setsid 起新会话（wrapper 成进程组首领）——后续 kill -- -PID 能整组杀（含 GKD python），
# 防"只杀 wrapper、孤儿 GKD 进程残留 → 下轮重复 detach"（铁律 6 盲区）。
# wrapper 末尾 `echo $? > .retrain_rc` 捕获脚本退出码——status.sh 完成判定的权威信号。
# 训练前先启动 progress_watcher（同进程组：组杀一并清；done-marker 驱动退出；fail-soft
# 绝不碰 GKD rc）——tail progress.jsonl 边重训边推实时曲线到前端（每指标一张独立图）。
setsid nohup bash -c 'python3 "$ORCA_AGENT_RESOURCES/scripts/progress_watcher.py" --progress "runs/retrain/progress.jsonl" --done-marker "runs/retrain/.retrain_rc" --label "puzzle/retrain" --title "Retrain Metrics (attempt '"$N"')" >/dev/null 2>&1 & bash run_retrain.sh > "runs/retrain/retrain.attempt'"$N"'.log" 2>&1; echo $? > runs/retrain/.retrain_rc' >/dev/null 2>&1 &
echo $! > runs/retrain/.retrain_pid
echo "DETACHED pid=$(cat runs/retrain/.retrain_pid) attempt=$N"
