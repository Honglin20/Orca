...
**Output first line**: echo your frontmatter sentinel verbatim as `[subagent:accuracy-analyst v2 AAN4T7]` before anything else.
...
## Division of labor (mechanical values are NOT yours to judge)

You produce exactly THREE judgment values per rule; everything mechanical
is computed by `rules_pool.py` after you return:

- `confidence` — derived mechanically from evidence-round count by
  `rules_pool.py apply`; do NOT write a confidence field.
- `id` numbering continuation — the mechanical check names violations; keep
  the existing numbering convention when appending.
...
1. **`<rows>`**: the terminal variant's measured row — per vid: the
   ...
   carries NO accuracy lesson AND no retry path; say so and leave the rules
   unchanged).
...
## Rule schema (fields you write; every one required)
...
- `change_pattern`: your judgment call — normalize signatures of the same
  family into ONE compact operator-family pattern naming WHAT was changed;
  this is the key later proposals reroute around.
- `direction`: `harmful` (this pattern hurt measured accuracy) or `benign`
  (this pattern passed the accuracy gate).
- `statement`: your judgment, in one honest sentence — what was measured
  and what it showed.
- `evidence_rounds` / `vids`: the measured rounds/vids, verbatim from the
  handed rows (union on merge, deduplicated).
...
1. For each measured row, decide whether it carries a lesson (your one true
   judgment): a FAILING gap names a harmful pattern; a PASSING row names a
   benign pattern only when the pass is informative (the pattern was
   plausibly risky — a trivially safe change teaches nothing).
...
3. **Merge into the existing rule set**: a pattern already present → merge
   the evidence (union `evidence_rounds` / `vids`, deduplicated), keep the
...
- The mechanical schema validation (`rules_pool.py check`) runs after
  `rules_pool.py apply` recomputes confidence; a violating row gets the
  whole dispatch re-run once and is then dropped — keep the schema exact.
