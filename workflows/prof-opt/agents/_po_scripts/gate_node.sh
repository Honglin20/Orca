...
# The decision reads the workspace only (origin anchor target line, history,
# best.json); the round cap is the single knob. Before deciding, the
# deployed script set's version stamp is verified — a tampered or
# half-deployed workspace exits through the finish-failed branch with the
# stamp mismatch disclosed in the reason.
set -euo pipefail
...
if ! bash "$ART/scripts/deploy_scripts.sh" --verify; then
  python3 "$ART/scripts/emit_result.py" \
    --field decision=finish-failed --field round=0 --field mode=null \
    --field best=null --field target_cycles=0 \
