...
- **`supernet_summary.md`**: supernet training viability, recorded evaluation paradigm, KD decision, and NAS decisions made upstream.
...
- **`<user_project_root>`** (original project): authoritative source for training semantics including data pipeline, preprocessing, transforms, loss, metrics, optimizer, scheduler, and training budget baseline. Consult it through targeted, manifest-guided reads; when it corrects or extends the manifest, write the fix back into `project_manifest.md`.
