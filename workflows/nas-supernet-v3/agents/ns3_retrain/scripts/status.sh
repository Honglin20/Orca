#!/bin/bash
# status.sh —— ns_retrain 状态确定性判定（完成 + 存活 二合一，agent.md Step 1）。
# 输出一行判定（互斥）：
...
if [ -s .ns_retrain_ckpt_resolved.txt ]; then
  M="$(cat .ns_retrain_ckpt_resolved.txt | tr -d '\r\n ')"
  [ -n "$M" ] && CKPT="$M"
fi
...
" "$CKPT" 2>/dev/null | grep -q CKPT_VALID; then
  ...
  printf '%s' "$CKPT" > .ns_retrain_ckpt_resolved.txt
  printf 'retrain completed (rc=0, process exited, ckpt valid): %s' "$CKPT" > .ns_retrain_assessment.txt
