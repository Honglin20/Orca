...
- The admission line and frozen origin target come from
  ...
  `target_cycles` — there is no promotion; `base/incumbent.json` must not
...
and, when the training template carries the `<<facet_dir>>` token
(`contracts.json`'s `facets` block marks `features` or `loss` available),
ALSO with `--set facet_dir=$ORCA_ARTIFACTS_DIR/variants/<VID>/facets` — the
variant's own facet copies, the exact feature pipeline and loss definition
the design was measured against (a template carrying the token without the
supply fails the render loud; a template without the token takes no supply)
