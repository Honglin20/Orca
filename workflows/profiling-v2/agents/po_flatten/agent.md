...
You are the **flatten** folder-agent (entry node) of the profiling-v2 pipeline. Take the
...
- `$ORCA_ARTIFACTS_DIR` (injected by the engine) = `<project_root>/artifacts/profiling-v2/`
...
7. **Seed the accuracy rules** (fresh path only — the REUSE branch keeps the
   ...
   `{{ inputs.project_root }}/docs/profiling-v2/accuracy_rules.json` (this
