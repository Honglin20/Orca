...
- The report archive directory is fixed at `docs/profiling-v2` relative to
  `{{ inputs.project_root }}`. At the END of the builder, copy the run's
  human-readable deliverables into it: `profiling_v2_report.md`, the `charts/`
...
- opens the report's first section with the four disclosure lines:
  ...
  (the training device backend every training ran on), the chart
  daemon state (`.chart_push.log` last line + this run's pushed
  dictionary — offline/failed written down), and the facet capability
  matrix (`contracts.json`'s `facets` block: whether `features` / `loss`
  were available and exercised, with the dry-run evidence path — an
  unavailable facet narrowed the search space and must be visible here),
  plus the deployed scripts'
...
re-verification → facet drift re-verification against
`contracts.json.facets.origin_hashes` → winner's variant-shadow diff PLUS
the winner's variant facet-copies diff → new-file names
`<stem>_profiling_v2<suffix>` for up to three grouped pieces (model file,
feature facet files, loss facet files — a piece whose facet was unavailable
is simply absent) → conflict entries → byte-verification).
The user's existing files are never modified. The report states that the
written pieces form ONE wiring group: adopting the model file without its
feature facet files (when the winner carried them) yields a model that
cannot run.
...
`{{ inputs.project_root }}/docs/profiling-v2/accuracy_rules.json` with the
...
+ self-contained `dashboard.html`, and `profiling_v2_report.md` at the
...
- **The report's structural gate runs FIRST** (the builder just wrote
  `profiling_v2_report.md`; a failure here means the builder's report is
