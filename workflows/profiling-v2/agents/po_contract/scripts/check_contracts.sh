...
#                   be compared). v2: the facet capability fingerprint
#                   rides the same track — every facets.origin_hashes
#                   entry's user-project file is re-hashed, and the
#                   $ART/facets/ copy must be present with the recorded
#                   hash (any mismatch -> exit 3 + fresh_start; the
#                   VERSION_KEYS facets discriminator rejects a block-less
#                   contracts.json the same way).
#                   Hard errors -> 2.
...
VERSION_KEYS = {
    ...
    "facets": lambda c: _facets_block_ok(c.get("facets")),
}


def _facets_block_ok(b) -> bool:
    """v2 §1: the facet capability block IS a version discriminator — a
    workspace whose contracts.json predates the block was built by an older
    contract stage and must not be reused. origin_hashes is the WRITER's
    flat shape (po_contract agent document): {<origin absolute path>:
    sha256} — the origin-to-copy pairing is derived in the reuse check
    below from the project-root-relative feature_files/loss_files lists."""
    if not isinstance(b, dict):
        return False
    for key in ("features", "loss", "export_consumes_features"):
        if not isinstance(b.get(key), bool):
            return False
    return (b.get("facets_dir") == "facets"
            and isinstance(b.get("feature_files"), list)
            and isinstance(b.get("loss_files"), list)
            and isinstance(b.get("origin_hashes"), dict)
            and isinstance(b.get("evidence"), str)
            and bool(b["evidence"]))
...
if mode != "--reuse-check":
    ...
    # v2 §4: <<facet_dir>> presence is pinned PER TEMPLATE to the facets
    # capability — train: features OR loss; eval: features (the eval render
    # feeds the feature pipeline only, never loss); export:
    # export_consumes_features. Token ⇔ capability, BOTH directions: a
    # token-less template in a capable workspace renders origin facets into
    # a variant run, and a token in an incapable workspace can never be
    # supplied (render_run fails loud on the unreplaced token).
    facets_cap = c.get("facets") if isinstance(c.get("facets"), dict) else {}
    feat_cap = facets_cap.get("features") is True
    loss_cap = facets_cap.get("loss") is True
    export_feat = facets_cap.get("export_consumes_features") is True
    for tname, tokens, facet_face in (
        ("run_full_finetune.template.sh",
         ("<<python>>", "<<epochs>>", "<<out_dir>>", "<<seed>>", "<<device>>"),
         feat_cap or loss_cap),
        ...
        if not tpath.is_file():
            ...
            has_facet = "<<facet_dir>>" in body
            if facet_face and not has_facet:
                problems.append(
                    f"templates/{tname} lacks <<facet_dir>> but the facets "
                    f"capability (features={feat_cap} loss={loss_cap} "
                    f"export_consumes_features={export_feat}) requires the "
                    f"token on this template (v2 §4)")
            if has_facet and not facet_face:
                problems.append(
                    f"templates/{tname} carries <<facet_dir>> without the "
                    f"matching facets capability — the render call sites "
                    f"supply it only when the capability exists (an "
                    f"unsuppliable token fails loud at render time)")
...
# ── v2 facet capability fingerprint (the REUSE face, same track as the
#   profile parameters): recompute the user-project facet file sha256 from
#   facets.origin_hashes and verify the $ART/facets/ copy is present with
#   the recorded hash — any mismatch means the workspace's facet basis
#   drifted after the contract, which needs fresh_start (never a partial
#   patch). facets/ lives OUTSIDE shadow/, so the lock/enumeration/assert
#   mechanisms are untouched by design (§0).
# origin_hashes shape (pinned by the po_contract agent document, the ONE
# writer): flat {<origin absolute path>: sha256}. The origin-to-copy pairing
# rides the located-file lists — feature_files/loss_files are
# project-root-relative and the copy preserves that structure, so each rel
# pins exactly one origin (suffix match on the normalized abs path; an
# ambiguous or missing pairing is itself a torn contract). An EMPTY map is
# the legal all-unavailable state (nothing located, nothing copied). ──────
if isinstance(c.get("facets"), dict) and mode == "--reuse-check":
    facets_b = c["facets"]
    hashes = facets_b.get("origin_hashes")
    rels = sorted({str(p) for p in list(facets_b.get("feature_files") or [])
                   + list(facets_b.get("loss_files") or [])})
    if not isinstance(hashes, dict):
        version_problems.append("facets.origin_hashes must be an object "
                                "{origin abs path: sha256} (the facet "
                                "fingerprint)")
    else:
        if rels and not hashes:
            version_problems.append(
                "facet files are located (feature_files/loss_files) but "
                "facets.origin_hashes is empty — the fingerprint is missing")
        if hashes and not rels:
            # the mirror tear: a fingerprint over files no list locates —
            # the copy-side pairing below would silently skip everything
            version_problems.append(
                "facets.origin_hashes is non-empty but feature_files/"
                "loss_files locate nothing — the located-file lists and the "
                "fingerprint disagree")
        for key, recorded in sorted(hashes.items()):
            if not isinstance(key, str) or not isinstance(recorded, str):
                version_problems.append(
                    f"facets.origin_hashes entry is malformed (needs "
                    f"path -> sha256 strings): {key!r}")
                continue
            src = Path(key)
            if not src.is_file():
                version_problems.append(f"facet origin file missing: {src}")
                continue
            if sha(src) != recorded:
                version_problems.append(
                    f"facet origin sha256 drift (the user project file "
                    f"changed after the contract): {src}")
        for rel in rels:
            rel_n = rel.replace("\\", "/")   # same normalization both sides
            if Path(rel).is_absolute() or ".." in Path(rel).parts:
                version_problems.append(
                    f"facet file escapes the facets dir: {rel!r}")
                continue
            matches = [k for k in hashes
                       if isinstance(k, str)
                       and k.replace("\\", "/").endswith("/" + rel_n)]
            if len(matches) != 1:
                version_problems.append(
                    f"facets.origin_hashes does not pin exactly one origin "
                    f"for the located facet file {rel!r} ({len(matches)} "
                    "matching absolute paths)")
                continue
            copy = art / "facets" / rel
            if not copy.is_file():
                version_problems.append(f"facets copy missing: facets/{rel}")
            elif sha(copy) != hashes[matches[0]]:
                version_problems.append(
                    f"facets copy hash mismatch (the workspace copy was "
                    f"edited): facets/{rel}")
