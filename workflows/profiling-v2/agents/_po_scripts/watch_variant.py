...
PY = ""
PROJ_ROOT = ""
FACET_DIR = ""      # v2: the variant facets dir supply for eval renders (""=absent)
EPOCHS = 1
DIRECTION = ""
SHADOW_PKGS = ""
CKPT_RULE = ""
...
def _load_contracts() -> None:
    ...
    global PROBE_K, WARMUP_FRAC, STREAK_FRAC, PROJ_ROOT, FACET_DIR
    ...
    try:
        ...
        # v2 §4 render point 4: eval must feed the VARIANT feature pipeline
        # (train/eval same pipeline = metric comparability). The supply is
        # conditional on the workspace capability — a missing facets block
        # means the templates carry no <<facet_dir>> token, so an empty
        # supply keeps token ⇔ supply consistent without failing here.
        facets = c.get("facets")
        if isinstance(facets, dict) and (facets.get("features") is True
                                         or facets.get("loss") is True):
            FACET_DIR = str(ART / "variants" / VID / "facets")
        else:
            FACET_DIR = ""
...
def render_eval(ckpt: str, out_rendered: Path, log: Path) -> None:
    sets = [f"ckpt={ckpt}", f"log={log}",
            f"shadow_dir={VDIR / 'shadow'}",
            f"shadow_pkgs={SHADOW_PKGS}",
            f"project_root={PROJ_ROOT}",
            f"python={PY}"]
    if FACET_DIR:
        # v2 §4: the eval template carries <<facet_dir>> iff the workspace
        # has a usable facet — supply exactly then (final_eval AND k_eval)
        sets.append(f"facet_dir={FACET_DIR}")
    args = ["bash", str(ART / "scripts" / "render_run.sh"),
