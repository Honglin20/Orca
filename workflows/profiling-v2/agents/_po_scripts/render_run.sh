...
#   facet_dir — OPTIONAL, --set ONLY (v2 §4): $ART/facets for origin-side
#   renders, $ART/variants/<vid>/facets for variant-side ones. Supply is
#   per call site and conditional on the template carrying <<facet_dir>>
#   (token presence ⇔ the corresponding facets capability — pinned by
#   check_contracts.sh). A template with the token and no --set supply
#   fails loud below like any unreplaced token; a supplied key for a
#   token-less template is simply unused.
#
