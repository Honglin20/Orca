...
Validates profiling_v2_report.md BEFORE the node relays the builder's JSON:
...
def main() -> int:
    ...
    ns = ap.parse_args()
    report = Path(ns.artifacts) / "profiling_v2_report.md"
    if not report.is_file() or report.stat().st_size == 0:
        print("check_report: FAIL profiling_v2_report.md missing or empty "
