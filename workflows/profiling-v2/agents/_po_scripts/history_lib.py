...
v2: the impl row additionally carries the three facet phrases
(structure/feature/loss, ≤80 code points each); terminal rows never do —
the render layer joins them by vid.
...
IMPL_FIELDS = (
    ...
    # v2 facet phrases (SPEC §2.1): one-sentence summary | 未改 |
    # "n/a (facet unavailable)" — ≤80 Unicode code points, judged with len().
    # Phrases live ONLY on impl rows; terminal rows join by vid at render time.
    "structure_change", "feature_change", "loss_change",
)
FACET_PHRASE_CAP = 80          # code points, len() semantics
FACET_UNCHANGED = "未改"
FACET_UNAVAILABLE = "n/a (facet unavailable)"
LATENCY_FIELDS = (
...
def check_facet_phrase(value: object, field: str) -> str:
    """The one facet-phrase validator (single source for every writer/gate):
    a non-empty str within the 80-code-point cap. WHICH literal is legal for
    which facet state (未改 / n/a (facet unavailable) vs a real summary) is
    R4/R5 territory — the emit gate judges that against contracts.json; this
    checks only the type/shape invariant every writer shares."""
    if not isinstance(value, str):
        raise HistoryError(f"{field} must be a str facet phrase, "
                           f"got {type(value).__name__}: {value!r}")
    if not value.strip():
        raise HistoryError(f"{field} must not be empty or blank — the "
                           f"no-change literal is {FACET_UNCHANGED!r}")
    if len(value) > FACET_PHRASE_CAP:
        raise HistoryError(f"{field} exceeds the {FACET_PHRASE_CAP}-code-point "
                           f"cap ({len(value)}): {value!r}")
    return value
...
def append_implemented(path: str | Path, vid: str, *, round: int, seq: int,
                       ...
                       structure_change: str = FACET_UNCHANGED,
                       feature_change: str = FACET_UNCHANGED,
                       loss_change: str = FACET_UNCHANGED) -> dict:
    ...
    The three facet phrases (v2 §2.1) mirror declaration.json verbatim;
    未改 is the default for a facet the design did not touch."""
    fields = {
        ...
        "structure_change": check_facet_phrase(structure_change,
                                                "structure_change"),
        "feature_change": check_facet_phrase(feature_change,
                                              "feature_change"),
        "loss_change": check_facet_phrase(loss_change, "loss_change"),
    }
