...
label+title on every push -> the front end replaces the previous chart):
  ...
  * line   ``profiling-v2/curves``  — top-10 training curves (§10.1): the baseline
  ...
  * pareto ``profiling-v2/pareto``  — every variant as one point (§10.2): x = the
  ...
  * table  ``profiling-v2/docs``    — the analysis-docs manifest (§10.4, ``--docs``):
...
_DEFAULT_LABEL = "profiling-v2/curves"
_BASE_TITLE = "profiling-v2 training curves"
_PARETO_LABEL = "profiling-v2/pareto"
_PARETO_TITLE = "profiling-v2 variants pareto"
_DOCS_LABEL = "profiling-v2/docs"
_DOCS_TITLE = "profiling-v2 analysis docs"
...
# v2 §2.3: the digest layer is retired — the mechanically derived
# base/history.md takes the manifest slot (same doc/status/path triple).
_DIGEST_ROW = ("digest", "history.md", "base/history.md")
...
def collect_docs(artifacts: Path) -> list[dict[str, Any]]:
    ...
    rows: list[dict[str, Any]] = []
    # v2 §2.3: the contracts.json facets capability decides the conditional
    # candidate row (feature.md) and is itself disclosed as a manifest row —
    # the capability matrix stays visible while running (silent narrowing
    # never hides). Best-effort read: a torn contracts.json only drops the
    # conditional row + the disclosure status.
    contracts = _read_json(artifacts / "contracts.json") or {}
    facets = contracts.get("facets")
    facets = facets if isinstance(facets, dict) else {}
    features_on = facets.get("features") is True
    candidate_names = ("semantic.md", "hardware.md", "sota.md")
    if features_on:
        candidate_names = candidate_names + ("feature.md",)
        ...
        for rdir in sorted((d for d in rounds_dir.iterdir() if d.is_dir()
            ...
            for name in candidate_names:
                if (r := row("round", name,
                    ...
                    rows.append(r)
    if (r := row("contract", "facets capability", "contracts.json",
                 (f"features:{'yes' if features_on else 'no'};"
                  f"loss:{'yes' if facets.get('loss') is True else 'no'}"))):
        rows.append(r)                                   # 能力矩阵披露（v2 §2.3）
    if (r := row(*_RULES_ROW, "snapshot")):              # 规则组（S-9 快照源）
        rows.append(r)
    if (r := row(*_DIGEST_ROW, "snapshot")):             # 叙事层（history.md 派生视图）
        rows.append(r)
    return rows
