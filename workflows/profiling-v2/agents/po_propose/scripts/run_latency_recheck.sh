...
#      (v8: shadow/ never moves — it is always the origin tree), plus the
#      v2 facet twin: $ART/facets vs variants/<vid>/facets through the same
#      diff_check file layer, diff == declaration.facet_edited_files
#      (skipped whole when the workspace has no $ART/facets —
#      structure-only);
...
for vdir in "$ART"/variants/*/; do
  ...
  # ---- layer 1b: facets file layer (v2 §3 Step 3 recheck facets 等值门) ----
  # $ART/facets vs variants/<vid>/facets through the SAME diff_check file
  # layer (the script itself is untouched — parameterization only); the
  # diff set must equal declaration.facet_edited_files exactly, the facet
  # twin of the shadow diff==edited gate. A workspace without $ART/facets
  # is structure-only: the layer skips (nothing to diff on either side).
  facet_rc=0
  if [ -d "$ART/facets" ]; then
    facet_edited="$("$PY" - "$decl" <<'PYEOF'
import json, sys
from pathlib import Path
try:
    d = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
except Exception as exc:
    raise SystemExit(f"declaration.json unreadable: {exc}")
v = d.get("facet_edited_files")
if not isinstance(v, list) or not all(isinstance(p, str) and p for p in v):
    raise SystemExit("declaration.json lacks facet_edited_files (v2 schema: a list of facets-relative paths)")
print(json.dumps(v))
PYEOF
)" || { echo "FATAL: $vid declaration.json facet_edited_files unreadable or not a list (see the python error above)" >&2; exit 2; }
    facet_out="$("$PY" "$SCRIPTS/diff_check.py" --layer file \
        --base-shadow "$ART/facets" --variant-shadow "$vdir/facets" \
        --edited-files "$facet_edited")" || facet_rc=$?
    [ "$facet_rc" -lt 2 ] || { echo "FATAL: facet file layer hard error for $vid" >&2; exit 2; }
    if [ "$facet_rc" -eq 1 ]; then
      fnote="$("$PY" -c 'import json,sys; v=json.loads(sys.stdin.read()); print("facet: not_declared=" + ",".join(v["not_declared"]) + " declared_but_absent=" + ",".join(v["declared_but_absent"]))' <<<"$facet_out")"
      mismatch_layers="$(append_layer_note "$mismatch_layers" "$fnote")"
    fi
  fi

  structural="pass"
  if [ "$file_rc" -ne 0 ] || [ "$facet_rc" -ne 0 ]; then
    structural="fail"
  fi
