...
You are the **baseline** node of the profiling-v2 pipeline. Four things make
...
- Upstream state read by the chain: `contracts.json` (interpreter,
  ...
  extraction), its `facets` block (the chain's training/eval renders ALWAYS
  bind the origin facet copies — the `facet_dir` supply points at the
  workspace-root `facets/`; the baseline never touches variant facet
  copies), `train_device.json` (training backend + count — the probe
...
works. Do NOT wait for the training to finish. Each analyst derives its
conditional feature chapters from `contracts.json`'s `facets` block itself
(an available facet adds the corresponding chapter; both unavailable adds
none) — `check_baseline_docs.sh` enforces exactly that conditional
required set, never a blanket pass.
