...
1. **`<output_dir>`**: the workflow workspace (`$ORCA_ARTIFACTS_DIR`) —
   ...
   your directions must be NEW families, not repeats),
   `accuracy_rules.json` when present (measured-harmful patterns to avoid),
   and `contracts.json`'s `facets` capability block — when it marks a facet
   available, also the corresponding copies under `facets/` (feature
   pipeline files, loss definition files).
...
- **body**: EXACTLY these four `##` sections, in this order, each with
  ...
  conditional facet chapter below when it applies:
...
**Conditional fifth chapter — `## 特征信息量与 loss 现状`**: written if
and only if `contracts.json`'s `facets` block marks at least one facet
available (the chapter set follows the capability matrix mechanically;
both unavailable → the chapter is entirely absent — never a placeholder).
Content, grounded in the `facets/` copies you opened, covering exactly the
available facets: the features side — per feature (or feature group) the
information it carries (the transform / pairing / aggregation it
implements), what would break if it were dropped or approximated, which
features are information-redundant or approximable, and how the feature
set's information content constrains input-side designs; the loss side —
what the current loss definition actually optimizes in information terms,
how it couples to the business logic (which error structure it penalizes
and which it ignores), and its weaknesses as accuracy-side levers (e.g. no
pressure against feature redundancy, miscalibrated margins). This chapter
is the idea source for feature-side designs and for loss-side judgment.
