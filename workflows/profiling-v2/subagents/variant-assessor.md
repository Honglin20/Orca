...
1. **`<output_dir>`**: the workflow workspace (`$ORCA_ARTIFACTS_DIR`). Read
   ...
   structure — describe it as it is), the VARIANT's facet copies
   `variants/<vid>/facets/` when present (the changed feature pipeline /
   loss definition — part of what you assess; diff them against the origin
   `facets/` copies), `project_manifest.md` and
   `contracts.json` (`model_facts`, and the `facets` block for which faces
   exist) for construction context, and the BASE
...
5. **`<change_note>`**: the variant's change description (the proposal's
   `change_sig` / `change_spec` / `rationale` plus its facet declarations —
   the `facets` block and the three facet phrases — what was changed, on
   which faces, and why).

## Method

Read the two baseline documents FIRST, then the variant source (the changed
modules and their neighbors) and, when present, the variant's facet copies
(the changed feature pipeline / loss files and their origin counterparts).
...
sacrificed. The soft-alignment scope INCLUDES the facet files: what the
facet phrases declare must match what the facet edits actually do, and the
feature pipeline's output must still feed the model's (possibly adapted)
input the way the design claims — a facet edit that contradicts its declared
phrase, or a declared face with no matching edit, is a finding exactly like
a shadow-edit contradiction.
...
2. **`## 输入输出`** — the variant's input/output spec; a structural change
   ...
   A design carrying the features facet may legitimately change the INPUT
   spec (describe the new input contract and how the feature pipeline
   produces it); the OUTPUT contract is always preserved — a variant whose
   output semantics shifted is a finding, whatever its numbers.
