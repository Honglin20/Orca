...
## ⑤ 2026-09-09 作者人工复核补记（独立于本快照审查 agent 的第二轮）

创作方按契约 §8 通读法独立重跑全文逐行核对（非仅机械校验）：

- **F1、F2 已修复**：L137 改「评测工具的成本估算对小 shape 有已知偏差」；bound 标签矩阵行与 ascend.md §1 诊断栏统一为模板枚举的连字形（cube满载 / vector-bound / memory-movement-bound）。
- **人工复核新发现 2 处（审查 agent 漏判），已同批修复**：
  - **R1（L193-194，v2 遗留）**：披露段占位符「`<失败/告警/MFU>100% 估算偏差/…>`」中 `MFU>` 使 `<` 提前闭合、段尾再出现悬挂 `>`——LLM 填模板时解析歧义。已改写为无内嵌 `>` 的平衡占位符（「`<评测失败、告警、MFU 超 100% 的估算偏差、…>`」）。
  - **R2（L105）**：cube 窗口括注「Conv/MatMul/Gemm 类」与 ascend.md §2 权威表（Conv、MatMul、Gemm、Linear）存在集差——已补 Linear。
- 修后复扫：v2 哨兵全仓零残留、五组禁词零命中维持；verdict **维持 CLEAN**。
