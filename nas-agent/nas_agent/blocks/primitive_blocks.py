# nas_agent/blocks/primitive_blocks.py
import torch
...
class ElasticLayerNorm(nn.LayerNorm):
    ...
    def forward(self, x):
        if self.elementwise_affine:
            ...
            bias = (
                self.bias[: self.sample_hidden_size]
    ...
    def elastic_num_params(self):
        if not self.elementwise_affine:
            return 0
        bias_params = self.sample_hidden_size if self.bias is not None else 0
        return self.sample_hidden_size + bias_params


class ElasticRMSNorm(nn.Module):
    def __init__(self, *, super_hidden_size: int, eps: float = 1e-8):
        super().__init__()
        self.super_hidden_size = super_hidden_size
        self.eps = eps
        self.weight = nn.Parameter(torch.ones(super_hidden_size))
        self.sample_hidden_size = super_hidden_size

    def set_sample_config(self, *, sample_hidden_size: int):
        assert sample_hidden_size <= self.super_hidden_size, (
            "sample_hidden_size cannot be larger than super_hidden_size"
        )
        self.sample_hidden_size = sample_hidden_size
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        weight = self.weight[: self.sample_hidden_size]
        return F.rms_norm(x, (self.sample_hidden_size,), weight=weight, eps=self.eps)

    def get_active_subnet(self) -> nn.Module:
        size = self.sample_hidden_size
        sub_layer = nn.RMSNorm(
            size,
            eps=self.eps,
        ...
        with torch.no_grad():
            sub_layer.weight.copy_(self.weight[:size])

        return sub_layer

    @property
    def elastic_num_params(self):
        return self.sample_hidden_size
...
class ElasticLinear(nn.Linear):
    ...
    def forward(self, x):
        weight = self.weight[: self.sample_out_dim, : self.sample_in_dim].contiguous()
...
class ElasticConv1d(nn.Conv1d):
    ...
    def _get_active_weight(self) -> torch.Tensor:
        if self.super_groups == self.super_in_channels:
            return self.weight[: self.active_out_channels, :1, :].contiguous()

        active_in_per_group = self.active_in_channels // self.active_groups
        return self.weight[: self.active_out_channels, :active_in_per_group, :].contiguous()
...
class ElasticEmbedding(nn.Embedding):
    ...
    def forward(self, x):
        weight = self.weight[:, : self.sample_embedding_dim].contiguous()
        return F.embedding(
            x,
            weight,
            self.padding_idx,
...
class DepthwiseMultiplierConv2d(nn.Conv2d):
    """Marker for ``groups == in_channels`` and ``out_channels > in_channels``.

    Torch-Pruning handles ordinary and 1x depthwise convolutions directly.  A
    multiplier depthwise convolution needs a different input-channel mapping:
    pruning one input removes the complete corresponding output-filter block.
    """
...
class ElasticConv2d(nn.Conv2d):
    ...
    def _get_active_weights(self):
        ...
        return active_weight.contiguous(), active_bias
    ...
    def get_active_subnet(self):
        weight, bias = self._get_active_weights()
        conv_cls = (
            DepthwiseMultiplierConv2d
            if self.active_groups == self.sample_in_channels
            and self.sample_out_channels > self.sample_in_channels
            else nn.Conv2d
        )
        sub = conv_cls(
...
class ElasticBatchNorm2d(nn.BatchNorm2d):
    ...
    def get_active_subnet(self):
        if self.affine:
            ...
            device, dtype = self.running_mean.device, self.running_mean.dtype
        else:
            device, dtype = torch.device("cpu"), torch.float32
        ...
        with torch.no_grad():
            ...
            if self.track_running_stats:
                ...
                sub.num_batches_tracked.copy_(self.num_batches_tracked)
        return sub
...
class ElasticLayerNorm2d(nn.Module):
    ...
    Normalizes over the **channel dimension only** (i.e., per spatial position),
    equivalent to `nn.LayerNorm(C)` applied after permuting NCHW to NHWC.

    Use this module when the original model applies per-channel LayerNorm on
    2D feature maps (e.g., `LayerNorm(num_channels)` in ConvNeXt-style blocks).

    When the original model uses `nn.LayerNorm([C, H, W])` or any
    `normalized_shape` that includes spatial dimensions, use a standard
    `nn.LayerNorm` as a fixed (non-elastic) module instead, since the
...
def _compute_num_groups(num_channels: int, channels_per_group: int = 16) -> int:
    """Compute the number of groups for GroupNorm from a per-group channel count.

    Finds the largest divisor of `num_channels` that yields at least
    `channels_per_group` channels in every group.  Guarantees
    `num_channels % result == 0` (PyTorch hard requirement).

    Special cases:
      - `channels_per_group >= num_channels` → returns 1 (LayerNorm behavior)
      - `channels_per_group = 1`             → returns `num_channels` (InstanceNorm behavior)
    """
    if channels_per_group >= num_channels:
        return 1
    target_groups = max(1, num_channels // channels_per_group)
...
class GroupNorm2d(nn.Module):
    """GroupNorm for NCHW feature maps with automatic group count adaptation.

    Wraps `nn.GroupNorm`.  The actual number of groups is derived from
    `channels_per_group` via `_compute_num_groups`.

    Special cases via `channels_per_group`:
      - `channels_per_group >= num_channels` → single-group GroupNorm.  Not
        equivalent to `nn.LayerNorm([C, H, W])`: the normalization statistics
        (mean/var over all C*H*W elements per sample) are identical, but affine
        parameters are per-channel `(C,)` instead of per-element `(C, H, W)`.
      - `channels_per_group = 1` → equivalent to InstanceNorm (IN)
    """
    ...
    def __init__(
        ...
        self.num_groups = _compute_num_groups(num_channels, channels_per_group)
        self.norm = nn.GroupNorm(
            self.num_groups, num_channels, eps=eps, affine=affine,
...
class ElasticGroupNorm2d(nn.Module):
    ...
    Uses `F.group_norm` directly so the actual number of groups can adapt
    when `sample_num_channels` changes.  The group count for a given channel
    width is `_compute_num_groups(feature_dim, self.channels_per_group)`,
    ensuring every subnet has the same per-group channel granularity.
    """
    ...
    def forward(self, x):
        ...
        actual_groups = _compute_num_groups(feature_dim, self.channels_per_group)
        ...
        return F.group_norm(x, actual_groups, weight=weight, bias=bias, eps=self.eps)
